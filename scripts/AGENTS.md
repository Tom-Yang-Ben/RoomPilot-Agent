...
Owners: Kai for the catalog, GLB/image delivery tools, `scripts/sql/`,
`scripts/runtime_catalog/` and `scripts/project_store/`; Cody/Ben for recognition
dataset tools; Bella for release/integration utilities.
...
Subfolders own one import lane each, and every importer keeps its schema as a
sibling `.sql` so `Path(__file__).with_name(...)` discovery stays deterministic:

- `scripts/sql/` — official furniture catalog and pgvector embeddings.
- `scripts/runtime_catalog/` — Phase 4 style cards, surfaces, renovation costs
  and quarantine (`roompilot_runtime_catalog_schema.sql`).
- `scripts/project_store/` — Phase 3 projects, render outputs and engineering
  persistence (`roompilot_project_store_schema.sql`).
