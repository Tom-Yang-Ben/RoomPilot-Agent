...
目前匯入內容是 8,557 筆官方家具、8,557 個 GLB、25,671 張三視角圖片紀錄，以及 7,958 筆 active／RAG-indexable 家具的 BGE-M3 向量。舊文件中的 9,349 筆家具、28,047 張圖片與 37,396 筆資產已不適用。
...
| 7 | 執行兩個 dry-run | 8,557 catalog 與 7,958 向量通過 |
| 8 | 建立資料庫並匯入家具 | transaction 與筆數核對成功 |
| 9 | 匯入 BGE-M3 向量 | 7,958 筆向量通過 |
...
只要有一個檔案缺少就停止正式匯入。Catalog、GLB manifest 與 GLB result 必須有完全相同的 8,557 個 item ID；圖片兩檔必須各有 25,671 列，代表每件家具三個視角。
...
`--require-all` 要求 7,958 筆 active／RAG-indexable 家具都有目前文字 hash 對應的向量。599 筆 inactive 家具不會進向量來源 view，也不應存在正式向量表。
...
| active furniture | 7,958 |
| inactive furniture | 599 |
| `furniture_categories` | 55 |
| `styles` | 6 |
| `furniture_styles` | 17,114 |
| `rooms` | 9 |
| `furniture_rooms` | 20,249 |
| current VLM annotations | 8,557 |
| `furniture_assets` | 34,228 |
| `furniture_quality_issues` | 1,647 |
| `furniture_catalog_current` | 7,958 |
| `furniture_catalog_api_current` | 7,958 |
| `furniture_embedding_source_current` | 7,958 |
...
- [ ] `.env` 已建立且未加入 Git。
- [ ] 家具 dry-run 顯示 8,557 件家具與 34,228 筆資產來源。
- [ ] 向量 dry-run 顯示 7,958 筆 BGE-M3 向量。
- [ ] 正式匯入的 transaction 與筆數核對成功。
- [ ] API current view 為 7,958 筆，inactive 家具為 599 筆。
