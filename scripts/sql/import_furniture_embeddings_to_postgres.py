...
script without ``--embeddings`` therefore creates/verifies the SQL contract and
the 7,958-row active source view while leaving ``furniture_embeddings`` empty.
...
EXPECTED_CATALOG_COUNT = 8_557
EXPECTED_SOURCE_COUNT = 7_958
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--embeddings",
        type=Path,
        help="含 item_id 與 embedding array 的 JSON 或 JSONL；省略時只建立／驗證空表與來源 view。",
    )
    ...
    parser.add_argument(
        "--require-all",
        action="store_true",
        help="要求 7,958 件 active/RAG-indexable 家具都有向量；正式完整批次建議啟用。",
    )
    parser.add_argument(
        "--allow-unnormalized",
        action="store_true",
