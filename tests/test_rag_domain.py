...
def _settings(
    ...
    *,
    ...
    return RagSettings(
        ...
        parser_model=model,
        parser_models=(model,) if provider == "openrouter" else (),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("openrouter", "OPENROUTER_API_KEY", "tencent/hy3:free"),
    ],
)
def test_settings_select_only_the_configured_rag_parser(
    ...
    monkeypatch.setenv("OPENROUTER_MODEL", model_name if provider == "openrouter" else "")
    monkeypatch.setenv("ROOMPILOT_RAG_OPENROUTER_MODELS", "")
...
def test_openrouter_parser_uses_json_object_without_fallback(tmp_path: Path) -> None:
    plan = _plan()
    response = SimpleNamespace(
        choices=[
            SimpleNamespace(
    ...
    completions = _FakeChatCompletions(response=response)
    ...
    assert parsed.plan.room_type == "living_room"
    assert parsed.usage == {"input_tokens": 12, "output_tokens": 24, "total_tokens": 36}
    assert completions.kwargs["model"] == "tencent/hy3:free"
    assert completions.kwargs["response_format"] == {"type": "json_object"}
    assert "server-secret" not in str(completions.kwargs)

    with pytest.raises(RagDependencyError, match="OPENROUTER_API_KEY"):
        parse_openrouter_query(
            "沙發",
    ...
    with pytest.raises(RagUpstreamError, match="failed"):
        parse_openrouter_query(
            ...
            settings,
            client=SimpleNamespace(
                chat=SimpleNamespace(completions=_FakeChatCompletions(error=TimeoutError()))
            ),
        )
...
def test_configured_parser_rejects_unknown_provider(tmp_path: Path) -> None:
    ...
    with pytest.raises(RagDependencyError, match="must be openai, anthropic, or openrouter"):
