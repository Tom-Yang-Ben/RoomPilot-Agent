...
def test_openrouter_parser_repairs_incomplete_free_model_json(tmp_path: Path) -> None:
    settings = _settings(tmp_path, provider="openrouter")
    parsed = parse_openrouter_query(
        "臥室想要雙人床、書桌與衣櫃，淺木色",
        settings,
        client=lambda _payload, _headers: {
            "choices": [{"message": {"content": '{"room_type":"bedroom"}'}}],
            "usage": {"prompt_tokens": 3, "completion_tokens": 2, "total_tokens": 5},
        },
    )

    assert parsed.plan.room_type == "bedroom"
    assert {item.category_group for item in parsed.plan.items} >= {"bed", "desk", "wardrobe"}
    assert parsed.usage == {"input_tokens": 3, "output_tokens": 2, "total_tokens": 5}


def test_openrouter_parser_preserves_explicit_furniture_intent(tmp_path: Path) -> None:
    settings = _settings(tmp_path, provider="openrouter")
    parsed = parse_openrouter_query(
        "臥室想要雙人床與書桌",
        settings,
        client=lambda _payload, _headers: {
            "choices": [{"message": {"content": _plan().model_dump_json()}}],
            "usage": {},
        },
    )

    assert {item.category_group for item in parsed.plan.items} >= {"sofa", "bed", "desk"}


def test_openrouter_fast_plan_keeps_questionnaire_style_and_furniture_groups() -> None:
    from backend.spatial_data.rag.openrouter_parser import build_fast_plan

    parsed = build_fast_plan(
        "room=bedroom; style:japanese; bed desk office_chair wardrobe lighting"
    )

    assert parsed.plan.room_type == "bedroom"
    assert parsed.plan.styles == ["japanese"]
    assert {item.category_group for item in parsed.plan.items} >= {
        "bed", "desk", "office_chair", "wardrobe", "lighting"
    }
...
def test_configured_parser_rejects_unknown_provider(tmp_path: Path) -> None:
    ...
    with pytest.raises(RagDependencyError, match="openai, anthropic, or openrouter"):
