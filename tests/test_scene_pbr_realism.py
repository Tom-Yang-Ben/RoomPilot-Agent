...
def test_confirmed_step4_door_never_moves_to_a_nearby_parallel_wall_gap() -> None:
    result = run_workflow_script(
        f"""
        import {{ doorOpeningForWallTopology }} from {json.dumps(ARCHITECTURE_MODULE.as_uri())};
        const opening = doorOpeningForWallTopology(
          ...
          {{
            id: "door-3", step4_confirmed: true, width_cm: 100,
...
def test_opening_edges_do_not_receive_wall_junction_caps() -> None:
    ...
    assert "protectedOpenings" in junctions
...
def test_gap_window_wall_sections_end_flush_with_the_opening() -> None:
    ...
    assert "wallSectionSpan(from, to, length)" in segment_walls
...
def test_dxf_wall_mass_is_extruded_before_segment_fallback() -> None:
    ...
    standalone = source.split("function buildStandaloneOpeningAssemblies", 1)[1].split(
        "function buildStructuralMembers", 1
    )[0]
    assert '"window-wall-sill"' in standalone
    assert '"window-wall-header"' in standalone
    assert '"door-wall-header"' in standalone
