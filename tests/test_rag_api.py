...
def test_rag_job_api_queues_multiple_room_requests(monkeypatch) -> None:
    """Later rooms must queue behind local embedding work instead of receiving 429."""
    _clear_rag_jobs()
    monkeypatch.setattr(rag_api, "RAG_SERVICE", _FakeService())
    client = TestClient(app)

    created = [
        client.post("/api/rag/search/jobs", json={"query": f"room-{index}", "top_k": 3})
        for index in range(3)
    ]

    assert [response.status_code for response in created] == [202, 202, 202]
    assert all(
        _wait_for_job(client, response.json()["job_id"])["status"] == "completed"
