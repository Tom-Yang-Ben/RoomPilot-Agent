...
def test_manual_scale_confirmation_replaces_low_confidence_ocr_scale() -> None:
    ...
    assert result["scale"]["distance_m"] == 6.3
    assert result["scale"]["m_per_px"] == 6.3 / 415
...
def test_recognition_presentation_summarizes_rooms_and_only_prompts_uncertain_findings() -> None:
    ...
    result = run_workflow_script(
        ...
        const presentation = buildRecognitionPresentation({{
          spatial_report: {{
            ...
            rooms: [
              {{ room_id: "bedroom-1", room_type: "bedroom", label: "主臥室", confidence: {{ score: 0.95, level: "high" }}, inner_dimensions_m: {{ width: 3.2, depth: 3.6 }}, net_area_m2: 11.52, polygon_m: [] }},
              {{ room_id: "balcony-1", room_type: "balcony", label: "陽台", confidence: {{ score: 0.62, level: "medium" }}, inner_dimensions_m: null, net_area_m2: null, polygon_m: null }},
            ],
    ...
    assert result["rooms"][0]["dimensionLabel"] == "3.20 × 3.60 m｜11.52 m²"
