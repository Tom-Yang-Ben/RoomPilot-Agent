...
| 一鍵重建 | `pgvector/pgvector:pg17`＋空 volume 首次自動還原 dump；`pg_isready` healthcheck | FR-066；根目錄 `docker-compose.yml:14-39`（`db` 服務；原本另一份只起 DB 的 `docker_postgresql/docker-compose.yml` 已於 2026-08-22 合併進來） |
...
| ~~OPEN-43~~ **已實測（2026-08-22）** | 向量筆數在契約間有 8,076 與 7,958 兩個數字 | **live DB 實測為 8,076**：`SELECT count(*) FROM roompilot.furniture_embeddings` → 8076；依 `embedding_model` 分組只有 `BAAI/bge-m3` 一組，`embedding_dimension` 全為 1024，`count(DISTINCT item_id)` = 8076（每件恰一條）。索引僅 pkey／unique(item,model,text_hash)／`idx_furniture_embeddings_item_model`，**確認無 HNSW**，符合 §6 匯入拒收條件。7,958 未出現在本 dump | [`ADR-008`](../03_architecture/adr/ADR-008-rag-retrieval-only-offline-models.md)、[`test_plan.md`](../05_qa/test_plan.md) |
...
| ~~待確認（無既有 OPEN）~~ **已修（2026-08-22）** | Docker 還原路徑不一致：compose 掛載 `./scripts/sql/roompilot_db_dump.sql.gz`，該相對路徑以 compose 檔所在的 `docker_postgresql/` 解析而下無此檔 | 改掛整個資料夾（`docker-compose.yml:33`），postgres 只執行 `*.sql`／`*.sql.gz`／`*.sh`。空 volume 實跑還原成功：`furniture_catalog_api_current` 與 `furniture_catalog_current` 皆 8076、`vector` extension 已啟用 | [`deployment_and_operations.md`](../06_ops/deployment_and_operations.md) §4.1 |
