...
| TC-053 | ACPT-053 | FR-061、FR-062、NFR-013 | 整合＋人工 | `tests/test_design_manual_api.py:111`（離線八章）、`:149`（LLM 可用時潤稿）、`:176`（產出後可下載）、`:214`（未產出回 404）；`tests/test_delivery_proposal_api.py:287`（PDF 端到端，需 Playwright）、`:340`（引擎缺席回 503）、`:276`（離線文案誠實回報） | 綠 |
