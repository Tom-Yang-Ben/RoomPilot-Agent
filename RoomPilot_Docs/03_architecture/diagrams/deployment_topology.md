...
| PG | `pgvector/pgvector:pg17`，埠 `${DB_PORT:-5432}`，`pg_isready` healthcheck，空 volume 首次自動還原 dump；用戶端預設 `localhost:5432/roompilot_db`、池 1–8、連線逾時 3 秒、`DB_SSLMODE` 預設 `disable`；第 6 步讀 view `roompilot.furniture_catalog_current`，不可用時 `/api/catalog/status` 回 `available=false`（RB-001） | `docker-compose.yml:14-39`；`postgres_repository.py:20,194-196,211-224,226-245` | MOD-SQL、MOD-CAT |
...
| 3 | PostgreSQL 正式部署形態（Docker vs 原生安裝）未拍板 | 本文件新增（無既有 OPEN） | `docker_postgresql/DOCKER_ONECLICK.md:44-45` 保留原生安裝路徑。**原併列的「首次自動還原掛載路徑對不上」已於 2026-08-22 修復並實跑驗證**：改掛整個 `docker_postgresql/` 資料夾（`docker-compose.yml:33`），空 volume 首次啟動還原成功（8076 筆、`vector` extension 已啟用），見 [`deployment_and_operations.md`](../../06_ops/deployment_and_operations.md) §4.1 | [`runbook-catalog-db-unavailable.md`](../../06_ops/runbook-catalog-db-unavailable.md)、[`deployment_and_operations.md`](../../06_ops/deployment_and_operations.md) |
