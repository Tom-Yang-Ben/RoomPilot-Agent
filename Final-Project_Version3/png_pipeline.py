...
def _classify_opening(ink_clean: np.ndarray, axis: str, pos: float, thickness: float,
    ...
    # 超寬縫隙（> 門洞尺寸上限）：只有「連續貫穿的窗線」能成立為窗——
    # 整面牆的長窗即此類。門不可能這麼寬，側區又太深（會掃到家具），
    # 故不做門弧判斷、不交深度學習，非窗即略過（回 opening，呼叫端不輸出）。
    if wide:
        return "window" if window_hint else "opening"
    ...
    # 判斷順序（門弧最優先——閉合門扇常畫在牆帶內、會偽裝成窗線，
    # 但「開啟弧」只屬於門，是唯一不會出現在窗上的特徵）：
    #   1. 一側有明顯開啟弧 → 門（即使牆帶內有線，那是門扇不是窗）
    #   2. 牆帶內有平行細線且無門弧 → 窗
    ...
    if window_hint:
        return "window"
    if tpl_ok and tpl_kind == "window":
        return "window"
    if tpl_ok and tpl_kind == "door":
        return "door"
    return "opening"
