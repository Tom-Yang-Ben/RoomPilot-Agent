...
# 門與窗一律以「紅字紅線」呈現長度（門的辨識與呈現跟窗完全一致）；牆與開口另有色
_COLORS = {"wall": "#17313B", "door": _DIM_COLOR, "window": _DIM_COLOR, "opening": "#8A6D1D"}
...
def _element_svg(e: dict, stroke_w: float, font_size: float, offset: float,
                 placer: _Placer, show_label: bool = True) -> str:
    x1, y1 = e["p1"]
    x2, y2 = e["p2"]
    ...
    return line + _placed_text(placer, lx, ly, label, font_size, _DIM_COLOR, 700, rot)
...
def _furniture_svg(item: dict, font: float, placer: _Placer,
    ...
    return rect + _placed_text(placer, x + w / 2, y - fs * 0.4, label, fs, color, 600, extra=tag)
...
def _room_box_svg(r: dict) -> str:
    """空間範圍框：虛線沿空間輪廓（牆體內緣）繪製。

    輪廓來自泛洪／分水嶺區域——止於牆邊、不越牆、與鄰區不重疊；
    開放處的分水嶺邊界即自動補上的隱形牆。無輪廓資料時退回外接矩形。
    """
    poly = r.get("poly")
...
def _room_svg(r: dict, font: float, placer: _Placer,
              show_name: bool = True, show_dims: bool = True) -> str:
    """空間標籤：名稱（show_name）＋面積／區間／外框（show_dims）。
    ...
    parts = []
    if show_name:
        parts.append(_text(cx, cy - fs1 * 0.7, name, fs1, _ROOM_COLOR,
                           extra=f' data-room="{r["id"]}"{tag}'))
    if show_dims:
        parts.append(_text(cx, cy + fs2 * 0.6, line2, fs2, _ROOM_COLOR, 600, extra=tag))
        parts.append(_text(cx, cy + fs2 * 1.9, line3, fs2, _ROOM_COLOR, 600, extra=tag))
    return "".join(parts)
...
def render_png_svg(result: dict, background: str = "original",
                   show_dims: bool = True, show_names: bool = True) -> str:
    """PNG 標註圖。

    background='original' 疊原圖；'structural' 疊屏蔽後結構圖。
    show_dims=False → 隱藏所有長度尺寸（元素標籤、全長、空間面積、格線註記）。
    show_names=False → 隱藏辨識名稱（空間名稱、家具類別）。
    """
    ...
    # 全長標註一律排在圖面外側：先排車道，再據以決定留白寬度（隱藏尺寸時不排）
    tot_base, tot_step = font * 2.0, font * 2.4
    planned = _plan_totals(result.get("edge_totals", []), w, h) if show_dims else []
    ...
    room_parts = [_room_svg(r, font, placer, show_names, show_dims) for r in rooms]
    elem_parts = []
    for e in result["elements"]:
        stroke = max(3.0, (e.get("thickness_px") or t) * 0.55)
        elem_parts.append(_element_svg(e, stroke, font, offset, placer, show_dims))
    furn_parts = [_furniture_svg(item, font, placer, show_names)
