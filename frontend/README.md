...
- **選擇平面圖**：下拉選 `data/testdata/pic/temp/` 既有的 DXF，或上傳 **DXF／PNG**（P0，20 MB 以內）。原檔與辨識結果都會綁定專案；JPG/JPEG/BMP 暫不列入 P0 UI。
...
- **AI 自動配置 2D 家具**：需求確認後逐房選件並由 `backend.engine` 以公分計算合法座標。OpenRouter 必須另行勾選同意，而且只能從伺服器候選 ID 提建議、不能輸出座標；未同意或服務不可用時走本機房型規則。
...
- **18 色卡與家具保護規則**：問卷明確選過的家具型號保持不變；未指定家具依新色卡換成同類型風格候選，且重新走 `backend.engine` 驗證位置。
