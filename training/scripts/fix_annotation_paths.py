...
# House 以 id（非 class）辨識的群組。CubiCasa 格式讓同名 id 重複出現，
# Inkscape 存檔強制 id 唯一會把重複者改名成 g2/g3…——牆/門直接從 GT 消失。
# class 屬性 Inkscape 不動，可據以還原。
ID_KINDS = ("Wall", "Railing", "Window", "Door")


def restore_ids(doc):
    """被 Inkscape 唯一化的 id 從 class 還原；回傳還原數。"""
    n = 0
    for e in doc.getElementsByTagName("g"):
        cls = e.getAttribute("class").strip()
        if cls in ID_KINDS and e.getAttribute("id") != cls:
            e.setAttribute("id", cls)
            n += 1
    return n
...
def fix_svg(svg_path, check_only):
    ...
    converted, errors = [], []
    restored = restore_ids(doc)
    changed = restored > 0
    if restored:
        converted.append(f"id還原×{restored}")
    for e in doc.getElementsByTagName("g"):
