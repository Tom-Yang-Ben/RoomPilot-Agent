...
def test_cubicasa_house_still_cannot_read_new_vocabulary():
    ...
    sys.path.insert(0, os.path.join(
        ...
        "training/CubiCasa5k"))
    # training/CubiCasa5k 是本機自管 checkout（gitignore 排除，換機走 training.zip）；
    # 沒有它的機器（如 ben 端）明確 skip，不誤報紅。
    floortrans = pytest.importorskip(
        "floortrans", reason="training/CubiCasa5k 本機 checkout 不存在")
