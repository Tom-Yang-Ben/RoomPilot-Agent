"""書房(office)／樓梯(stair) 兩類新增——全鏈路測試。

背景：CubiCasa 的 12 個 room class 沒有這兩類，`Office` 與 `StairWell` 在
`rooms_selected` 都是 11(Undefined)，語意投票（層 1/2）結構上產不出來。
因此本組測試涵蓋的是「證據層可不可以獨立把它們叫出來」與
「GT 側不再把它們倒進 space 混合桶」兩件事。
"""
import cv2
import numpy as np
...
def test_ocr_short_key_needs_exact_match():
    ...
    assert fp.ocr_room_label("GARDEN") != "office"
...
def test_ocr_text_names_office(tmp_path):
    """書房沒有可靠幾何證據（ASSET_KINDS 無書桌/書櫃），OCR 是唯一來源。"""
    det, labels, rooms, cc_file = _mk(tmp_path, texts=["office"])
    fp.classify_rooms_cc(det, labels, rooms, cc_file)
    assert rooms[0]["label"] == "office"


def test_new_kinds_do_not_leak_into_other_rooms(tmp_path):
    """無證據時不得憑空冒出 office/stair（0 分播種不是 0.15 門檻的免死金牌）。"""
    det, labels, rooms, cc_file = _mk(tmp_path)
    fp.classify_rooms_cc(det, labels, rooms, cc_file)
    assert rooms[0]["label"] not in ("office", "stair")
...
def test_eval_classes_include_new_kinds():
    assert "office" in CLASSES and "stair" in CLASSES
    assert len(CLASSES) == len(set(CLASSES)) == 11
...
def test_norm_label_passes_new_kinds_through():
    assert norm_label("office") == "office"
