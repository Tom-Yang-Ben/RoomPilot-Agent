...
切割後、命名前的三道整形（皆軸對齊，房間只有直角矩形系——使用者
域約束）：
1. `_merge_nondoor_bridges()`：走道橫斷橋合併（門弧＋門墨水雙重否決）
2. `_carve_stairs()`：開放區嵌樓梯以踏板串外框切出成房（宿主須 ≥4×
   足跡；同座梯段含隔平台 <140cm 先併）
3. `_split_by_text_anchors()`：一房含 2+ 房名文字錨點＝作者標了多個
...
- 切割：own_dataset 開發集 77.1%（121/157）、own_eval 保留集 76.4%
  ——v2.23 六輪＋硬化循環後開發集 +15 間、保留集持平。殘餘主因：
  無文字開放區誤併（floor47/52/54 型，OCR 錨點吃不到）、保留集
  floor64 的中距開口誤併（大核順封被解耦拿掉的已知代價，全域加寬
  封口實測淨負向不可行）、floor74 命名層對切割邊界敏感。
