...
async def manual_opening(
    ...
    """使用者自訂開口：使用者給位置（沿牆兩點），系統給尺寸（三層精度階梯）。

    回傳 {opening, tier, case, saved_sample, candidates}；驗證不過回 422 附原因。
    candidates＝同型批次掃描的相似位置（同一張圖同種符號畫法一致，一次確認全標）。
    """
    data = await file.read()
    if not data:
        raise HTTPException(400, "檔案為空。")
    from png_pipeline import measure_manual_opening
    try:
        out = measure_manual_opening(data, x1, y1, x2, y2, kind,
                                     mm_per_px=mm_per_px, source_name=file.filename)
    except Exception as exc:
        raise HTTPException(500, f"手動開口量測失敗：{exc}")
    if out.get("error"):
...
def retrain() -> JSONResponse:
    ...
    同時重訓門窗開口 CNN（door/windows 樣本增強、固定驗證集棘輪比較）。
    """
    from furniture_ml import request_retrain
    out = request_retrain()
    try:
        import opening_ml
        out["opening"] = opening_ml.retrain()
    except Exception as exc:
        out["opening"] = {"ok": False, "message": f"門窗模型重訓失敗:{exc}"}
    return JSONResponse(out)
...
def ml_status() -> JSONResponse:
    ...
    try:
        import opening_ml
        info["opening"] = opening_ml.model_info()
    except Exception:
        info["opening"] = {"available": False}
...
async def analyze(
    ...
    manual_openings: Optional[str] = Form(None),
) -> JSONResponse:
    ...
    if manual_openings:
        import json as _json
        try:
            parsed = _json.loads(manual_openings)
        except ValueError:
            raise HTTPException(400, "手動開口（manual_openings）不是有效的 JSON。")
        if not isinstance(parsed, list):
            raise HTTPException(400, "手動開口（manual_openings）需為陣列。")
        mo = parsed[:100]
    try:
        if name.endswith(".dxf"):
            ...
            # 隱藏長度尺寸圖（只留名稱）／隱藏辨識名稱圖（只留尺寸）
            result["svg_no_dims"] = render_png_svg(result, show_dims=False)
            result["svg_no_names"] = render_png_svg(result, show_names=False)
