...
CLASSES: dict[str, dict] = {
    ...
    # 樓梯：樓梯間指向＋R4 矛盾攔截線索（樓梯不會在臥室裡）。
    # 無樣板時不會自動偵測，由使用者框選定義後參與空間投票與訓練。
    "stairs":     {"zh": "樓梯", "room": "樓梯間", "w": 0.95, "size": (150, 600), "short": 70, "thr": 0.60, "confuse": []},
    # 單椅：斜擺單椅常無法辨識（步驟 5）——先註冊類別，使用者框選累積
    # 樣本後按「重新訓練模型」即納入辨識；權重 0.50＝僅供佐證，不單獨定案
    "armchair":   {"zh": "單椅", "room": "客廳", "w": 0.50, "size": (55, 110), "short": 45, "thr": 0.60, "confuse": ["sofa"]},
}
