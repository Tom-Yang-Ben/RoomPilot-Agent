...
── 為什麼需要轉換:兩邊座標系與牆體表示不同 ──────────────────────────
  ...
  因此本模組:① 平移到角落原點;② 從 wall_polys 的環邊還原成 Wall 線段;
             ③ width/depth 取自選定範圍;④ z 軸直接對應引擎的 y 軸(同為平面第二軸)。
...
# 環邊當「薄牆」的厚度(m)。環本身已是實心牆的內牆面,取薄值只是讓家具與牆面留一點
# 縫、不要正貼;取太厚會把可用空間吃掉。
DEFAULT_WALL_SEG_THICKNESS = 0.06


def _ring_area(ring: list) -> float:
    """多邊形面積(shoelace,絕對值,m²)。ring 為 [[x,z],...],首尾是否重複皆可。"""
    a = 0.0
    n = len(ring)
    for i in range(n):
        x1, y1 = ring[i]
...
@dataclass
class DxfRoomBuild:
    """轉換結果。room 給引擎用;offset 讓你把家具座標映射回平面座標:
    plan_x = pos_x + offset[0]、plan_z = pos_y + offset[1](接 3D 全景時用得到)。"""
    room: Room
    offset: tuple      # (ox, oz):選定範圍在平面座標中的最小角
    mode: str          # 實際採用的模式:"largest" / "plan" / "plan(fallback)"
    source_area: float  # 選定範圍面積(m²)
...
def build_room_from_dxf(
    ...
    """dxf_parser 的輸出 dict → DxfRoomBuild(含 Room + 座標映射資訊)。"""
    polys = parsed.get("wall_polys") or []
    holes = [h for p in polys for h in (p.get("holes") or []) if len(h) >= 3]
    ...
    bb = parsed["bbox"]
    ox, oz = bb["minx"], bb["minz"]
    walls = []
    for p in polys:
        ext = p.get("exterior") or []
        if len(ext) >= 3:
            walls += _walls_from_ring(ext, ox, oz, wall_seg_thickness)
        for h in (p.get("holes") or []):
            if len(h) >= 3:
                walls += _walls_from_ring(h, ox, oz, wall_seg_thickness)
    room = Room(width=round(bb["maxx"] - ox, 3), depth=round(bb["maxz"] - oz, 3), walls=walls)
