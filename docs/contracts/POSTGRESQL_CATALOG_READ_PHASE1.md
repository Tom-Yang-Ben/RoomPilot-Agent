...
- `/api/furniture/{item_id}` 使用 `item_id` 主鍵查詢，不再掃描 9,349 筆 Python list。
...
5 份正式匯入來源的家具 ID 皆為 9,349 筆；正式總表與 API view 也都應為 9,349 筆。`jp-armchairs-01-underl-tta-vacuum-flask-black-1-2-l`（UNDERLÄTTA Vacuum flask）已從 JSON items、四份 manifest、正式總表與 staging 移除，只在 JSON `excluded_items` 保留排除原因；AWS 實體檔目前不屬於任何正式資料路徑。

## Phase 1 驗收清單

- [x] PostgreSQL 總表與 API view 都是 9,349 筆家具，誤分類保溫瓶不存在於 SQL。
- [x] 9,349 筆啟用家具各有 1 個 ready/uploaded GLB URL。
- [x] 9,349 筆啟用家具各有 front / side / angle-45 三張 ready/uploaded 圖片 URL（共 28,047 張）。
...
Live 驗收結果：`/api/furniture` 回傳 9,349、篩選查詢成功、詳情回傳 200、模型端點回傳 CloudFront 307，`/api/catalog/status` 回報 9,349 個 GLB 與 28,047 張三視角圖片。
