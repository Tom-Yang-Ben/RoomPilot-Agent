...
第 8 步側欄「生圖細節（可留白）」收六個 3D 場景表現不出來的軸：間接光、
天花分區、送風方式、維修口、吊扇、廚房家電外露。前端 `RENDER_DETAIL_FIELDS`
是唯一定義處，已解析成中文才送出。

控制項放在第 8 步而不是第 5 步：第 5 步的初談問卷明講「家具、材質與設備留到
方案階段再談」，而且第 5 步的「全屋風格與設備」panel 自 `614ae3a4` 起就被
`rp-legacy-questionnaire[hidden]` 以 `display: none !important` 關閉，放在那裡
使用者永遠看不到。第 8 步是按下生圖之前，也是這幾項唯一會被用到的地方。
