...
家具 catalog／embedding 可由 `scripts/sql/` 完整重建。既有 runtime catalog tables/views 與 project adapter 可讀，但 Phase 3 的 `scripts/project_store/`、Phase 4 的 `scripts/runtime_catalog/` 目前缺失；這兩部分尚不具備新環境從零重建能力。
...
Catalog 與 runtime catalog 在正式環境及未設定時均採 strict PostgreSQL：
...
Project store 是獨立邊界：正式部署必須明確設定 `ROOMPILOT_PROJECT_STORE_PROVIDER=postgres`，但目前程式為相容舊環境，在未設定時仍預設 `sqlite`。一旦明確設定 PostgreSQL，連線失敗不得靜默 fallback。
...
`scripts/runtime_catalog/import_runtime_catalogs_to_postgres.py` 目前不存在；既有資料庫可讀，但不可從本 repo 重建。恢復後才可重新加入 dry-run 與正式匯入命令。
...
`tests/test_postgres_single_source_phase5.py` 目前大部分行為測試可執行，但 schema 版本測試會因缺少 `scripts/runtime_catalog/roompilot_runtime_catalog_schema.sql` 失敗，因此尚不能作為全綠門檻：

```powershell
.\.venv\Scripts\python.exe -m pytest -q tests/test_postgres_single_source_phase5.py
```

Live PostgreSQL 與 hot-refresh 的原驗證包含 `tests/test_runtime_catalog_phase4.py`；目前該測試會因缺少 `scripts.runtime_catalog` 在 collection 階段停止。恢復 Phase 4 schema/importer 後才重新啟用：
