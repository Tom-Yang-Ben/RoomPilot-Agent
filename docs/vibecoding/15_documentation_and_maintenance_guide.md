...
- `!docs/vibecoding/` + `!docs/vibecoding/*.md` 兩行與 `docs/vibecoding/` 全套文件已於 2026-07-26 commit `d88b707` 一併入庫(git log 實測)。
...
- **版本控制**:契約檔頭維護「最後更新」日期;vibecoding 檔頭維護基準分支與日期(統一格式「基準分支 bella-local-20260726 | 2026-07-26」;程式基準 commit e48cd67 記於 13/14 內文)。
...
分工邊界:README 負責安裝與啟動,總覽負責跨模組協作(兩檔開頭互相聲明)。模板建議的「貢獻(CONTRIBUTING.md)」「授權」段落現況皆無:repo 無 `CONTRIBUTING.md`,但根層已有 `LICENSE` 檔(GPL-3.0,git ls-files 實測已入版控,README 無「授權」段落指向它),合併規範直接寫在 README「合併方式」段——README 是否補「授權」段待裁決。
...
1. **隨開發同步撰寫**:欄位或行為變更與契約更新在同一次合併內完成(總覽「合併與驗證」段「修正 import 與契約,最後驗證」);不要事後補文檔。
...
4. **文件聲明必須可複測**:寫入路徑、路由、指令、數字前先用工具查證;查不到就標「(未查證)」或「待補」——本套件(vibecoding 01–17)全部依此規則產出。
