...
>
> - **原始來源**：`origin/cody` 分支根目錄的 `MAIN_SYNC_TODO.md`。
> - **原作者**：峙宏。Git 作者欄位顯示為整合帳號 `Tom-Yang-Ben`，實際撰寫者依團隊分工為峙宏。
> - **原始提交**：`b14a347d52e03134a8aa1c16b5d9b27ebf62d7ba`（短碼 `b14a347`），提交時間 2026-07-27 02:01，
>   提交主旨為「refactor(scripts): scripts/ 解散——main 必需三支入 backend/floorplan、研發工具退出版控」。
>   這是該檔在 cody 分支上最後一次被修改的提交。
> - **本檔為原文保存，內容未經修改。** 下方本文一字未動，包含已經過時的段落，
>   因為這份文件的價值在於保留交接當下的原貌。
>
> **已查證的一處過時記載**：本文第 1 點稱主線現況的權重預設是
> `training/model_best_val_loss_var.pkl`。實測主線 `backend/floorplan/vision/cody_semantic.py`
> 第 23 行為 `DEFAULT_WEIGHTS = Path("training/model_finetuned_v5.pkl")`，
> 主線早已切換到 v5 微調權重。因此本文第 1 點所述的落差已不成立，
> 現在兩邊只差目錄層級（主線在 `training/`、cody 期望在 `backend/floorplan/`），檔名相同。
