> **收編說明（由主線整理者補充，非原文的一部分）**
>
> - **原始來源**：`origin/cody` 分支根目錄的 `Readme.md`，是 cody 平面圖辨識管線的常駐架構說明＋版本變更紀錄。
> - **原作者**：峙宏。Git 作者欄位顯示為整合帳號 `Tom-Yang-Ben`，實際撰寫者依團隊分工為峙宏。
> - **原始提交**：`4f6c7e47`（cody 分支 2026-08-03 終版，最新版本節為 v2.33）。
>   前一版快照為 `b14a347d`（v2.17，2026-07-27），本次隨 cody-dev 合併輪替更新。
> - 文中提及的 `FloorTraining.md` 與 `training/` 研發工具**不隨交付**（終版裁定：
>   評測/重訓一律回 cody 分支操作），僅存在 cody 分支。
> - **本檔為原文保存，內容未經修改。** 下方本文一字未動；文首「現行管線架構」段落
>   隨 cody 程式碼更新，changelog 段落為歷史紀錄不回改。
> - 文中提到的路徑一律以 cody 管線的目錄視角書寫（backend/floorplan/、training/、
>   testdata/），與主線同構；引用前請自行對照主線現況。
> - 原引用的 `COLOR_PIPELINE_PLAN.md`、`SEG_FAILURE_ANALYSIS.md` 已於 2026-08-03
>   收尾移除，內容見 cody 分支 git 歷史與 changelog 版本節。

# 現行管線架構（常駐說明，隨程式碼更新；以下 changelog 為歷史紀錄不回改）

> **2026/8/3 案子收尾——截止版本 v2.33**（本檔 changelog 記至 v2.33 止；
> 重啟任務由此版本接續）。changelog **由新到舊**排列：最新的 v2.33 在
> 最上方，往下閱讀是回溯歷史（v2.4 在檔尾）。
...
標準口徑表；能力總覽見 `docs/recognition_report.html`。
...
- 切割：own_dataset 開發集 **82.2%（129/157）**、own_eval 保留集
  **80.6%（58/72）**、保留集核心 5 類 **86.7%**——v2.23+v2.24 全日
  ...
  詳見 changelog v2.23~v2.24 版本節（原 `SEG_FAILURE_ANALYSIS.md` 與
  `docs/superpowers/` 歸因文件已隨收尾移除，完整內容見 git 歷史）
...
2026/8/3 v.2.33 變更（美式素材外科啟用＋分割頭語意牆帶＋灰門 R 0.81——彩 dev 76.3% 新高）

### 一、變更

- **美式畫風符號素材輪**：使用者裁切 20 件入 Asset（trashcan 新類）、
  庫重建 943→961（basin 尺寸拓寬涵蓋方盤洗衣盆）；白/黑名單 npz 序號
  經 raster 逐位元映射。probe v225 全掃新模板 **0.8 門檻 17TP/0FP**，
  外科啟用 trashcan@0.8＋basin 白名單 +4，新機制 `TPL_THR`（逐模板
  chamfer 門檻覆寫——262 型「Bath TP ≤1.011/Kitchen FP ≥1.187」只有
  逐模板 1.1 能全收全擋）。floor13 收案（12px 線稿解析度天花板）。
- **彩色門/窗輸出閘**（使用者裁定）：`emit_openings=false` 預設關——
  彩門 P 0.38/彩窗 P 61 低於可用線，交付停畫、後端自建（json 帶
  `openings_suppressed` 標記）；偵測保留供封口/比例尺。「中央最近
  牆縫」選拔規則離線 A/B 否決（P 0.38→0.42、R 0.47→0.07）。
- **監督式分割頭（語意牆帶）**：白牆帶真/假三角度量測（手工 0.617/
  帶級 DINO 0.67/全域結構 0.64）證明牆帶表徵原理瓶頸後，DINOv2
  patch＋線性頭（19 張彩 dev GT 訓練，LOIO patch AUC 0.974/IoU
  0.876）機率圖低機率脊線 → 語意牆帶 → 白牆救援 flood。彩 dev
  **114→116**（floor_07 6→7、floor_08 3→4，其餘 17 張逐位元零變動）。
- **灰門範圍擦邊收割**：FN 逐樘分型（32 樘）——最大桶是範圍擦邊 12
  （74.3/105.5cm 差嚴窗 0.4~5.5cm）而非墨水閘（旁路實測 R 不動）。
  room json 門位輸出窗對齊 DXF 批次放寬窗 (70-110)+(150-200)，
  **R 0.71→0.81**（90/111）、兩條產品路口徑統一。
- 誠實負結果（皆 revert 收案）：floor52 廚房合併（儲藏室假廚符污染
  淨 -1）、floor04 尖端對縫（假封口 -3、無門虛擬分界原理不可攻）、
  halfgap 合併（單邊半縫、前提不成立零轉化）。

### 二、量測（標準口徑）

| 量尺 | 空間切割正確性 | 核心5項判讀率 |
| :--- | :--- | :--- |
| 灰階 dev（153 房） | 88.9%（136/153）持平 | 91.8%（112/122）持平 |
| 灰階 holdout（72 房） | 83.3%（60/72）持平 | 75.0%（45/60）持平 |
| 彩色 dev（152 房） | **76.3%（116/152）**（前 75.0%） | **74.5%（82/110）** |
| 彩色 holdout（62 房） | 69.3%（43/62）持平 | 66.7%（34/51）持平 |

彩 dev 口徑註記：分割頭訓練見過 dev 圖（比照分域雙頭慣例，無偏數字
＝LOIO patch AUC 0.974）；holdout 無災難張、語意牆帶未觸發（泛化待
新樣本回驗）。前版 changelog 彩 dev 曾記 72.4%（110/152），與循環三
之後的實測 114/152 口徑不一致，以本版 116/152 為準。

| 門/窗 | 灰階 | 彩色 |
| :--- | :--- | :--- |
| 門（產品 zones 路） | **P 0.83／R 0.81**（F1 0.82，與 DXF 批次路對齊） | 輸出關閉（偵測 P 0.38/R 0.47） |
| 窗 | P 98%／R 92% | 輸出關閉（偵測 P 61%/R 42%） |

門殘餘 FN：seamless 10（射線原理性搭不到，尖端對縫實測不可攻）、
halfgap 6（單邊半縫）、deadzone 3（110~150cm）、other 1。

---

2026/8/2 v.2.32 變更（門量尺對準產品 zones 路——灰 P 0.85/R 0.71；彩窗結構化判定第一刀）

### 一、變更
- **門偵測接線定案**：產品 room json 的 doors 欄位本就來自 build_rooms zones（門尺寸牆縫＋墨水證據），缺的是量測管路——新工具 `eval_door_zones.py` 直接量產品輸出路。全集實測：**灰 24 張 P 0.85/R 0.71**（vs 弧掃描路 P 0.38/R 0.17）；彩 P 0.38/R 0.47（thin=None 無墨水否決 FP 偏高，仍勝停用的 0，改善需彩圖墨水證據層另立）。
- **彩窗 covered_soft 結構化**：貫穿覆蓋 0.8→0.65（極淡線淡出斷裂適配）＋墨跡總量上限改「非貫穿線列殘墨 ≤0.25」。窗 P 63→61/R 40→42（F1 微升）、彩房間 dev 逐張持平。多數 FN 仍卡開口枚舉層，專輪續攻。
- Stair 約束層診斷收案（3 條粗踏板不等距、放寬撞衣櫃同構風險，維持不動）。

### 二、量測（標準口徑，房間指標與 v2.31 同）

| 量尺 | 空間切割正確性 | 核心5項判讀率 |
| :--- | :--- | :--- |
| 灰階 dev / holdout | 88.9% / 83.3% | 91.8% / 75.0% |
| 彩色 dev / holdout | 72.4% / 69.3% | 68.2% / 66.7% |

| 門/窗 | 灰階 | 彩色 |
| :--- | :--- | :--- |
| 門（產品 zones 路） | **P 0.85／R 0.71** | P 0.38／R 0.47 |
| 窗 | P 98%／R 92% | P 61%／R 42% |

---

2026/8/2 v.2.31 變更（三發子彈——{Entry,LivingRoom} 對、灰階收割回收＋DINO 種子補位；灰 dev 88.9%/holdout 83.3% 雙新高）

### 一、變更
- **提案器 {Entry, LivingRoom} 對**：floor52 實案（玄關半 DINO 0.62/客廳半 0.99 只因不在接受對而沒開刀）。從嚴 0.60、Entry 半須貼殼緣。灰 dev floor52 4→6/7。
- **灰階回收口袋收割**（自屬開關，灰階量尺獨立 A/B）：floor06 露台在殼外而灰階未開收割——早前分流時退場的 +3 依「灰階自己驗證自己啟用」原則回收。
- **灰階 DINO 種子補位**：符號種子路跑完後補一輪 DINO 種子合併（floor06 整圖只偵測到 kstove、浴室碎格無種子可併）。floor06 4→7/7、floor38 5→6/6 雙雙全中。
- **Stair 踏板診斷收案（零出貨）**：floor08/13/54 零踏板串，2x 重萃取實驗仍零——卡點在 _stair_runs 等距/數量約束層而非單純解析度，另立時間盒（EV 3 間）。
- 順手修 door_match/eval_door_match 的 cp950 編碼炸彈。

### 二、量測（標準口徑）

| 量尺 | 空間切割正確性 | 核心5項判讀率 |
| :--- | :--- | :--- |
| 灰階 dev（153 房） | **88.9%（136/153）**（前 85.0%） | **91.8%（112/122）**（前 88.5%） |
| 灰階 holdout（72 房） | **83.3%（60/72）**（前 81.9%，floor79 6/6） | **75.0%（45/60）**（前 73.3%） |
| 彩色 dev（152 房） | 72.4%（110/152）持平 | 68.2%（75/110）持平 |
| 彩色 holdout（62 房） | 69.3%（43/62）持平 | 66.7%（34/51）持平 |

### 三、門/窗辨識率（2026-08-02 現況重測）

| 項目 | 灰階 | 彩色 |
| :--- | :--- | :--- |
| 窗（eval_windows，答案集綠框） | **P 98%／R 92%**（121TP/3FP/10FN） | **P 63%／R 40%**（57TP/33FP/87FN，v2.13 首輪後未調參） |
| 門（eval_door_match，own GT 111 樘，score_fused ≥0.85） | **P 37.8%／R 17.1%** | 停用（doors=[]，僅借 detect_doors 抑制窗誤判） |

門的量測註記：本輪以現行管線＋door_match 融合重測；歷史 comment 中的「R 0.73/P 0.86」屬別條量測（refine_scale 開口候選脈絡），口徑不同不可比。門偵測現況遠低於可用線，列為獨立待辦。

---

2026/8/2 v.2.30 變更（Bath 碎格 DINO 種子合併＋口袋頸縮分裂；彩 dev 72.4%）

### 一、變更
- **浴室碎格合併 DINO 種子路**（彩圖限定）：彩圖 symbols=[] 使灰階的符號種子合併啞火。`_merge_bath_nooks` 加 seed_ids/veto_ids 參數路——DINO 判 Bath（≥0.7）的 <4m² 碎房當種子、判非 Bath 者與口袋房當反證；灰階符號路原樣。三迭代守門：0.5 門檻/8m² 會讓磁磚陽台亂併（-4）、已配上的浴室被吸稀釋（±0），0.7＋4m²＋口袋反證後乾淨 +1（floor_14）。
- **低紮實度口袋頸縮分裂**：solidity <0.35 的口袋 CC 以 T/2 erode 斷頸、核心回膨脹，窄條剝除死於面積門檻——floor_10（陽台黏殼緣窄條成 0.08 細長 L、IoU 0.49）與 floor_02（0.48）兩個近失案修復；保底兩道（0.35~0.5 不動、核心 <40% 整顆不動）。floor_13 -1（其鬆散口袋剝除後消失，三迭代未解，如實記錄）。
...
| 彩色 dev（152 房） | **72.4%（110/152）**（前 71.0%） | 68.2%（75/110） |
| 彩色 holdout（62 房） | 69.3%（43/62）持平（28 -1/29 +1 對沖） | 66.7%（34/51）持平 |

197 測試綠。彩 dev 未中 42 間中 17 間在棄守三張（07/08/09）內——可攻分母實際為 25 間。
