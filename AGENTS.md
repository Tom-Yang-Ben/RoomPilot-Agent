...
- 目前工作樹中的 `scripts/` 是本專案唯一有效的最新版本。處理腳本任務時，必須先讀取目前的 `scripts/AGENTS.md`、`scripts/README.md` 與目標檔案，再以現行內容為基準修改。
- 不得從遠端分支、Git 歷史、舊備份、下載副本、舊文件範例或帶版本尾碼的暫存檔，回填、還原或混入任何舊版腳本內容。
- 使用者提供新的完整 `scripts/` 版本時，該交付內容是唯一替換來源；先核對完整檔案清單、輸入輸出契約與 `git status --short`，再一致替換對應檔案，不得混用新舊實作。
- 只有最新交付清單明確未包含、且使用者已確認淘汰的檔案才能移除；不得依猜測刪除腳本。
- 替換完成後必須同步核對 `scripts/README.md`、相關 `docs/contracts/`、呼叫端與測試，並執行各腳本的 `--help`／dry-run、對應測試、`git diff --check` 與 `git status --short`。
