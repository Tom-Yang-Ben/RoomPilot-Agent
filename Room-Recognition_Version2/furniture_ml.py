...
def _train_once(prev: Optional[Model]) -> Optional[Model]:
    ...
    ys += ay + py
    if _HAS_TORCH:
        model, acc = _train_cnn(imgs, ys, classes)
    else:
        model, acc = _train_softmax_backend(imgs, ys, classes)
    ...
    if not deployed:
        _status["last_result"] = (f"v{version} 未通過驗證閘門"
                                  f"（黃金集 {cand_g:.1%} < 現役 {prev_g:.1%}），已退回")
        return None
    _save(model, acc, len(imgs), n_pseudo, version, cand_g, cand_n)
    _status["last_result"] = f"v{version} 已部署"
    return model
