...
1. **門偵測重建**（灰 P 37.8/R 17.1 遠低於可用線）：GT 111 樘齊、
   eval_door_match 量尺在（cp950 已修）；detect_doors 弧掃描＋
   door_match 融合鏈全面體檢。彩門停用中。
2. **彩窗專輪**（P 63/R 40）：微調已證死路（0.60 墨跡上限實測
   ...
   不受影響。
3. **Stair 約束層**（灰 3 間）：零踏板串、2x 重萃取仍零——卡
   _stair_runs 等距/數量約束，需 dump Hough 線逐步驗。
