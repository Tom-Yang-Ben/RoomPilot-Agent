...
- **彩門證據層收案（零出貨）**：四判別器全數實測不可分——迴轉區
  ...
  縫品質整體提升。
- **色塊分割原型（進行中）**——關鍵洞察：棄守畫風的牆＝「兩片色染
  之間的白色窄帶」，反向偵測白牆。已驗證步驟：(1) meanshift 平滑
  ＋chroma≤7/L>195 白遮罩；(2) top-hat（1.5T）剝出窄帶；(3) 方向
  性開運算（5T 長核）濾磁磚格噪點——floor_08 浴室區白牆現形。
  已知問題：(a) 臥室隔牆漏（meanshift 把牆帶染上鄰接木色，chroma
  閘卡掉→試 chroma≤12＋縮小平滑半徑）；(b) 殘餘假長帶（陽台磚
  列、圖框緣）待「兩側色染」驗證；(c) 下一步＝白牆帶→rects→併入
  detect_walls（同 hollow_wall_rects 的去重併法）→ 棄守三張 A/B。
