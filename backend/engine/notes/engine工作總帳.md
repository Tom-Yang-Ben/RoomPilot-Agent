...
### 0803 午後增補（五表對齊收斂＋沙發反邊案）

1. **Bartow 沙發反邊案結案**：換款時 `modelOrientationDeg=180` 被從 AmazonBasics 舊沙發
   沿用，但 Bartow GLB 出廠正向——舊 JS 不吃該欄位所以先前看似正常，重啟拿新 JS 才爆。
   rev 493 四處改回 0；**朝向標注是單顆 GLB 事實，換款必重驗**（已入 skill 反向表＋定版樣本）。
   另：晨間「從資料庫更換家具」把定版四件鎖洗掉（前端換件流程 bug，Bella 單），rev 492 重上鎖。
2. **使用者拍板「前端照策略表走」**：scene_layout2d 種子表（床頭櫃×2／餐椅×4／玄關獨立
   房名判斷＋鞋櫃）、scene_v2 問卷預設（臥室補床頭櫃、客廳補電視櫃、廚房清死鍵留白、
   儲藏換層架、浴室去 vanity 死鍵）全數對齊 v1。**「可選預設給」三件拍板**：客廳茶几／
   浴室鏡櫃／陽台植栽＝預設給、可取消（README 新節＋對帳工具 `OPTIONAL_DEFAULTS` 放行）。
3. **對帳工具修三洞**：問卷表 study 鍵未折 workspace（假缺 desk）、玄關「優先；不足退」
   逐字數成必備（假缺 2/3）、前端鍵未正規化。五表對帳 **8 偏離 → 1**（剩 entry：後端問卷
   詞彙未開放，SPACE_DEFAULTS／問卷無從掛鍵，Bella 單）。
4. **cache 戳雙層更新**：scene_v2 內 layout2d import 戳＋scene.html bundle 戳；依賴測試
   邊清單原漏 `scene_layout2d.js`，已補（不補則改模組不換戳、瀏覽器吃舊檔＝反邊案同病）。
   契約測試新增回歸鎖：玄關鞋櫃／餐椅×4／床頭櫃×2。
