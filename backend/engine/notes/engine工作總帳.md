...
- 2026-08-01 之後：新增擺放策略層，floor04 三間房**擺放成功 10 件、失敗 0 件、不該浮空卻浮空 0 件**。
- **但畫面不會變**，因為 `scene_service.py` 用的是它自己那張候選表，引擎只是備援（見 §5.1）。
...
| 提出日 | 擱置 | 事項 | 邊界 | 現況 |
|---|---|---|---|---|
| **07-29** | **3 天** | **沿牆／找位置的搜尋從 `scene_service` 收回引擎** | ⚠️ 跨 Bella | 引擎側已完成（08-01），**接線未做**。見 §5.1 |
| **07-29** | **3 天** | **美學／擺放評分**（first-fit → 評分選優） | ✅ engine 內 | 部分完成：策略層已能產生「有設計」的候選，但**還沒有評分排序**，仍是找到第一個合法解就停 |
| **07-29** | **3 天** | 窗前禁放帶從 `scene_service` 收回引擎 | ⚠️ 跨 Bella | 未做。引擎現在有窗台高度規則，但落地窗前的**禁放帶**還沒做（要先拍板 30 vs 60） |
| **07-29** | **3 天** | 地毯可壓、壁架略過碰撞 → 引擎層級規則（＝**堆疊層**） | ✅ engine 內 | **未做**。仍靠 `_IGNORE_COLLISION_TYPES` 偷吃步 |
| **07-29** | **3 天** | 走道連通檢查（人走不走得過） | ✅ engine 內 | 未做 |
| **07-29** | **3 天** | 清死鍵 `bed-frame`／`cabinets-cupboard` | ✅ engine 內（別名表跨 Bella） | 未做。見參考手冊 §2-E |
...
POST /api/scene/generate
  → scene_service.generate_layout()            :1396
      → scene_service._placement_candidates()  :717  ← 80 行寫死的候選位置＋角度表，17 種家具
      → 逐個候選丟 engine.check_placement_with_clearance() 驗證
      → 全部失敗才 → engine.place_furniture()  :1654  ← 引擎只是備援
```

`scene_service` 還擁有：`_WALL_ANCHORED_TYPES`（:697，貼牆清單）、`_WINDOW_CLEARANCE_EXEMPT_TYPES`、`forbidden_zones`（窗前禁放帶）、`_hinted_wall_candidate`。

👉 **07-29 就寫過**：「沿牆／網格找可放位置——大半 `scene_service`——『能不能放』的搜尋屬幾何」，並列為「適合收回 engine」。
