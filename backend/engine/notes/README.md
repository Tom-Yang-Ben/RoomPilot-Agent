...
| 檔案 | 說明 | 可否當正式規格 |
|---|---|---|
| `engine工作紀錄_2026-07-31.md` | **最新交接總帳**（環境建置／12 房型拍板／三邊串接評估／換環境交接） | 否；§4 拍板**已升級**至 `../room_strategy/README.md` v1 |
...
接手順序：先讀 `engine工作紀錄_2026-07-31.md` §0 →「§8.5 換環境交接」→ 依 §6 待辦執行。
**換到新機器時 §8.5 A 節必做**：git 不帶資料庫，每台機器都要自己建一次 PostgreSQL。
