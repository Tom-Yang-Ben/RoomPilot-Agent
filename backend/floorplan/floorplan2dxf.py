...
def run(cfg: Config):
    ...
    if cfg.style == "solid":                 # 實心牆：矩形 + SOLID HATCH；窗：符號
        ...
        root = os.path.dirname(os.path.dirname(cfg.output)) or "."  # 跟輸出目錄同層(data/testdata/dxf_scale、data/testdata/json)
...
def run_batch(in_dir: str, out_dir: str, cfg: Config):
    ...
    chk_dir = os.path.join(os.path.dirname(out_dir) or ".", "chk")  # 跟輸出目錄同層(data/testdata/chk)
