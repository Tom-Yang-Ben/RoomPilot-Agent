...
class hg_furukawa_original(nn.Module):
    ...
    def init_weights(self, pth=None):
        """MPII 姿態估計預訓練權重（Bulat et al.）——**訓練起點專用**。

        推論不呼叫：架構建好後呼叫端會 load_state_dict 換上 v5 微調權重，
        這裡載的參數 100% 被覆蓋。上游寫死相對路徑
        'floortrans/models/model_1427.pth'（部署端因此被迫 chdir 進
        training/CubiCasa5k）；本副本改成顯式參數，且不隨附那顆 70MB 檔。
        """
        from . import model_1427
        if pth is None:
            raise ValueError(
                "ccmodel 是推論期副本，不隨附 model_1427.pth（70MB，"
                "訓練起點用）。要預訓練請改用 training/CubiCasa5k/ 原版，"
                "或明確給 pth=<model_1427.pth 路徑>。")
