...
參數全部寫在 config_color.ini（彩色管線專用，與黑白管線的 config.ini 分離），執行只要：
    python3 floorplan2dxf_color.py              (批次 testdata/color_png/ → testdata/dxf/)
指定別的設定檔：
    python3 floorplan2dxf_color.py 別的.ini

輸出一律進 color 子目錄（temp/chk/color/ testdata/dxf/ temp/json/color/
temp/json/color_arch/），與主程式 floorplan2dxf.py 的 temp/chk/gray/ testdata/dxf/gray/
temp/json/gray/ temp/json/arch/ 完全分開，不互相覆蓋。
...
def load_config(path: str) -> Config:
    if not os.path.isabs(path) and not os.path.isfile(path):
        _alt = os.path.join(os.path.dirname(os.path.abspath(__file__)), path)
...
def write_json(path, img_w, img_h, rects, wins, doors, mm_per_px, info, cfg: Config,
    ...
    """前端交接 JSON：cm 座標與 testdata/dxf/ 完全一致(原點左下、y 向上)，
...
def write_arch_json(path, img_w, img_h, rects, wins, doors, mm_per_px, info,
    ...
    # temp/json/color/ 保留全部門(帶 score 給前端過濾)；temp/json/color_arch/ 直接蓋白模：只收高信心門，
...
def write_solid_dxf(rects, wins, img_h, scale, cfg: Config, out=None, insunits=4,
    ...
    out/insunits 給 testdata/dxf/ 用：另存一份門寬比例、公分單位(insunits=5)的檔。"""
    doc = ezdxf.new("R2010")
...
def run(cfg: Config):
    ...
    if not cfg.output:                       # 慣例：DXF → testdata/dxf/
        ...
        cfg.output = os.path.join(
            "testdata/dxf", os.path.splitext(os.path.basename(cfg.input))[0] + ".dxf")
...
def run_batch(in_dir: str, out_dir: str, cfg: Config):
    ...
    print(f"  JSON → temp/json/color/ (前端交接)   白模 → temp/json/color_arch/")


def main():
    p = argparse.ArgumentParser(
        description="彩色平面圖 PNG/JPG/BMP → DXF。參數見 config_color.ini；輸入/輸出可用指令覆蓋。\n"
                    "不帶參數 = 批次跑 testdata/color_png/ 目錄下所有圖檔 → testdata/dxf/ + temp/chk/color/")
    p.add_argument("input", nargs="?",
                   help="輸入圖檔 png/jpg/bmp(單張)；不給或給目錄則批次")
    p.add_argument("output", nargs="?",
                   help="輸出 DXF/目錄（不給就自動輸出到 testdata/dxf/同檔名.dxf）")
    ...
    if (not a.input and not cfg.input) or \
       (a.input and (os.path.isdir(a.input) or a.input.lower() in ("png", "color_png"))):
        in_dir = a.input if (a.input and os.path.isdir(a.input)) else "testdata/color_png"
        if not os.path.isdir(in_dir):
            sys.exit(f"找不到目錄 {in_dir}/  (請把要批次的圖檔放進去)")
        run_batch(in_dir, (a.output or "testdata/dxf"), cfg)
        return
    ...
    if not os.path.isfile(cfg.input):
        alt = os.path.join("testdata/color_png", cfg.input)
        if os.path.isfile(alt):
            cfg.input = alt
    base = os.path.splitext(os.path.basename(cfg.input))[0]
    if not cfg.preview:                      # 慣例：疊圖 → temp/chk/color/
        os.makedirs("temp/chk/color", exist_ok=True)
        cfg.preview = os.path.join("temp/chk/color", base + "_chk.png")
    run(cfg)
