...
輸出：
  ...
  training/json/room/<名>_room.json 房間清單（房型/面積/bbox/有無門/辨識證據）＋門位＋比例資訊
...
def process(path, out_dir, cfg_bw, cfg_color):
    ...
    os.makedirs("training/json/room", exist_ok=True)
    json_out = os.path.join("training/json/room", base + "_room.json")
...
def main():
    ...
    print(f"  JSON → training/json/room/  (<名>_room.json：房間+門位清單)")
