...
# Catalog room types remain the authoritative contract for server-side
# enrichment. Keep aliases here so Bella workflow payloads and Yen agent
# rules share the same ten RoomPilot room types.
...
def item_allowed_in_room(item: dict[str, object], room_type: object) -> bool:
    ...
    if not expected:
        return False
    if is_outdoor_item(item):
        return expected in OUTDOOR_ROOM_TYPES
    declared_rooms = item_room_types(item)
    if declared_rooms:
        return expected in declared_rooms
    return affinity_permits(str(item.get("normalized_type") or ""), expected)
