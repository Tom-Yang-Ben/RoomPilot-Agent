...
`look` 約 100–140 字。`rationale` 一到兩條，每條 `body` 60–90 字。`specs` 三到六列。`extra_images` 只給客廳、主臥這種重點空間，它會與主視覺並列成半版寬兩張。
...
`rooms[].extra_images` 會與 `hero_image` 並列在該章開頭（主視覺在左、附圖依序往右），兩張各佔半版寬——像客廳的日光／夜間，屋主要並排比才看得出差別。給第三張以上會換行往下疊，該空間就會被推到第二頁；只有真的值得多看一張圖的重點空間（通常是客廳、主臥）才放。並列時 `hero_caption` 也要講清楚左邊那張是什麼（例如「客廳日光。」），不能只寫「最終渲染」。
