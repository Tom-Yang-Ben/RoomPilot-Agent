...
def _repo_candidates(cache_dir: Path, repo_id: str) -> tuple[Path, ...]:
    return (
...
def _resolve_local_model_path(cache_dir: Path, repo_id: str) -> Path | None:
    for candidate in _repo_candidates(cache_dir, repo_id):
        if (candidate / "config.json").is_file():
            return candidate
        snapshots = candidate / "snapshots"
        if snapshots.is_dir():
            # Prefer the newest snapshot directory that has config.json.
...
def _repo_is_cached(cache_dir: Path, repo_id: str) -> bool:
    return _resolve_local_model_path(cache_dir, repo_id) is not None
    ...
    def _load(self, settings: RagSettings) -> tuple[Any, Any]:
        ...
        with self._load_lock:
            ...
            try:
                ...
                embed_path = _resolve_local_model_path(
                    settings.model_cache_dir, EMBED_MODEL
                )
                rerank_path = _resolve_local_model_path(
                    settings.model_cache_dir, RERANK_MODEL
                )
                if embed_path is None or rerank_path is None:
                    raise RagDependencyError("RAG model weights are not cached")
