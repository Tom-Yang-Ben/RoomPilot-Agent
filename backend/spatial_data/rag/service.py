...
from .openrouter_parser import build_fast_plan
...
class FurnitureRagService:
    ...
    def status(self) -> dict[str, Any]:
        ...
        provider_valid = provider in {"openai", "anthropic", "openrouter"}
        # OpenRouter uses the standard-library HTTP client so it has no package
        # prerequisite. OpenAI and Anthropic retain their explicit SDK checks.
        parser_package = (
            True if provider == "openrouter"
            else importlib.util.find_spec(provider) is not None if provider_valid else False
        )
        blockers: list[str] = []
    ...
    def search(
        ...
        parse_message = (
            "由問卷偏好建立快速檢索條件。"
            if request.fast
            else "由 LLM 將自然語言解析為受控條件。"
        )
        self._report(progress, 8, "parsing", parse_message)
        parsed = build_fast_plan(request.query) if request.fast else self._parser(request.query, settings)
