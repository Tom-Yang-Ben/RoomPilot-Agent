    ...
    try:
        with _borrow_connection(project_dir) as connection:
            with connection.cursor() as cursor:
                ...
                cursor.execute(
                    """
                    SELECT batch_key, MAX(loaded_at) AS imported_at,
                           MIN(source_file) AS source_file, COUNT(*) AS record_count
                    FROM staging.stg_furniture_catalog
                    GROUP BY batch_key
                ...
                cursor.execute(
                    ...
                    SELECT
                        ...
                        TO_REGCLASS('roompilot.projects') IS NOT NULL,
                        (SELECT MAX(updated_at) FROM roompilot.furniture_items),
                        CASE
                            WHEN TO_REGCLASS('roompilot.projects') IS NOT NULL
                            THEN (SELECT COUNT(*) FROM roompilot.projects)
                            ELSE NULL
                        END
                    """
                )
