...
from .scene_service import selection_complete_fn
...
# MAX_FLOORPLAN_BYTES / MAX_PROJECT_NAME_CHARS re-export：
# tests/test_api_input_hardening.py 直接 `from backend.server.main import` 取用；
# _read_upload 是 /api/upload（留在 main 的舊 DXF 端點）仍在用的共用 helper。
...
def _floorplan_ocr_provider():
    ...
    tests/test_ocr_wiring.py 以 monkeypatch.setattr(main, ...) 錨定本函式與
    default_ocr_provider，必須留在 main 命名空間；projects router 以 getter 取用。
    """
...
# 佇列 7 拆分第四批：專案 CRUD、平面圖與渲染路由群搬進 projects_api.py。
# PROJECT_STORE 被 conftest 與多個測試 monkeypatch 替換、user_store 依當下的
# store 解析、_floorplan_ocr_provider 被 test_ocr_wiring monkeypatch，一律以
# lambda 延遲解析 main 的模組全域，補丁語意不變。
app.include_router(
    build_projects_router(
        project_store_getter=lambda: PROJECT_STORE,
        user_store_getter=lambda: user_store(),
        floorplan_ocr_provider_getter=lambda: _floorplan_ocr_provider(),
        sample_floorplan_630=SAMPLE_FLOORPLAN_630,
    )
)
...
# /api/floorplan/analyze 與 /api/floorplan/confirm 已隨佇列 7 第四批搬進
# projects_api.py;其餘只剩測試在用,待確認無外部消費者後可一併移除。
# ---------------------------------------------------------------------------
