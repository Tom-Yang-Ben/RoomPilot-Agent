...
_ROOM_MATERIAL_TYPE_ALIASES = {
    "toilet": "bathroom",
    "washroom": "bathroom",
}
_WET_ROOM_FLOOR_TYPES = {"bathroom", "kitchen", "balcony"}
_WET_ROOM_FLOOR_CATEGORIES = {"tile", "wood_tile"}


def _validate_confirmed_material_workflow(workflow: dict) -> None:
    preferences = workflow.get("design_preferences")
    if not isinstance(preferences, dict) or not (
        preferences.get("confirmed") is True
        and preferences.get("materialsConfirmed") is True
    ):
        return
    rooms = (workflow.get("space_confirmation") or {}).get("rooms") or (
        workflow.get("recognition") or {}
    ).get("rooms") or []
    if not isinstance(rooms, list):
        return
    surfaces = {
        str(surface.get("surface_id")): surface
        for surface in load_surface_catalog().get("surfaces") or []
        if isinstance(surface, dict) and surface.get("surface_id")
    }
    whole_house = preferences.get("wholeHouse") or {}
    room_preferences = preferences.get("rooms") or {}
    ...
    def validate_surface(
        *,
        ...
        normalized_id = str(surface_id or "")
        surface = surfaces.get(normalized_id)
        invalid_reason = None
        if not normalized_id or surface is None:
            invalid_reason = "surface_not_found"
        elif usage not in (surface.get("usage") or []):
            invalid_reason = "usage_not_allowed"
        elif (
            usage == "floor"
            and room_type in _WET_ROOM_FLOOR_TYPES
            and str(surface.get("category") or "").lower()
            not in _WET_ROOM_FLOOR_CATEGORIES
        ):
            invalid_reason = "wet_room_floor_not_allowed"
        if invalid_reason:
            raise HTTPException(
                422,
                {
                    "code": "invalid_room_material_selection",
                    "room_id": room_id,
                    "surface": usage,
                    "surface_id": normalized_id,
                    "selection": "secondary" if secondary else "primary",
                    "reason": invalid_reason,
                    "message": "目前材質不適用於此空間，請重新選擇後再確認。",
                },
            )
    ...
    for room in rooms:
        if not isinstance(room, dict) or not room.get("id"):
            continue
        room_id = str(room["id"])
        room_type = _ROOM_MATERIAL_TYPE_ALIASES.get(
            str(room.get("type") or "").lower(),
            str(room.get("type") or "").lower(),
        )
        saved = room_preferences.get(room_id) or {}
        override = saved.get("surfaceOverride") or whole_house
        validate_surface(
            room_id=room_id,
            room_type=room_type,
            usage="wall",
            surface_id=override.get("wallSurfaceId"),
        )
        validate_surface(
            room_id=room_id,
            room_type=room_type,
            usage="floor",
            surface_id=override.get("floorSurfaceId"),
        )
        boundary = saved.get("materialBoundary")
        if not isinstance(boundary, dict):
            continue
        boundary_usage = str(boundary.get("surface") or "")
        if boundary_usage not in {"wall", "floor"} or not boundary.get(
            "secondarySurfaceId"
        ):
            continue
        validate_surface(
            ...
            surface_id=boundary.get("secondarySurfaceId"),
            secondary=True,
        )
...
def _page(name: str, *, headers: dict[str, str] | None = None) -> FileResponse:
    return FileResponse(STATIC_DIR / name, headers=headers)
...
@app.get("/questionnaire/{invite_token}")
def questionnaire_page(invite_token: str) -> FileResponse:
    _questionnaire_project(invite_token)
    return _page("questionnaire.html", headers=_questionnaire_private_headers())
...
def _questionnaire_project(invite_token: str) -> dict:
    try:
        return PROJECT_STORE.get_project_by_questionnaire_invite(invite_token)
    except KeyError as exc:
        raise HTTPException(404, "questionnaire_invite_not_found") from exc


def _questionnaire_private_headers() -> dict[str, str]:
    return {
...
_QUESTIONNAIRE_BASIC_OPTIONS = {
    "residents": {"adult", "child", "elderly", "pet", "frequent_guest"},
    "residentCount": {"one", "two", "three", "four", "five_plus"},
    "ageNeeds": {"none", "preschool", "school_age", "aging", "mobility"},
    "scheduleInterference": {
        "same_schedule", "noise_conflict", "light_conflict",
        "bathroom_conflict", "kitchen_conflict",
    },
    "homeWorkStudyCount": {"none", "occasional", "one_regular", "multiple_regular"},
    "homeWorkStudyNeeds": {
        "none", "video_calls", "quiet_focus", "simultaneous", "background_storage",
    },
    "futureChanges": {
        "stable", "new_child", "children_grow", "elderly_move_in",
        "work_change", "rent_or_resale",
    },
    "hostingFrequency": {"rare", "monthly", "small_regular", "large_regular"},
    "hostingNeeds": {"none", "meal", "large_gathering", "overnight", "children_visit"},
    "budgetPriority": {"budget_first", "balanced", "quality_first", "timeline_first"},
    "budgetRange": {"undecided", "under_100", "100_200", "200_350", "350_plus"},
    "targetTimeline": {
        "undecided", "within_3_months", "three_to_six_months",
        "six_to_twelve_months", "over_one_year",
    },
    "immutableNeeds": {
        "none", "existing_furniture", "appliance",
        "altar_or_piano", "fixed_pipes", "building_rules",
    },
}
_QUESTIONNAIRE_BASIC_EXCLUSIVE = {
    "ageNeeds": {"none"},
...
_QUESTIONNAIRE_MULTI_BASIC_KEYS = {
    "residents",
    "ageNeeds",
    "scheduleInterference",
    "homeWorkStudyNeeds",
...
_QUESTIONNAIRE_AXIS_PREFERENCES = {"a", "lean_a", "balanced", "lean_b", "b"}
_QUESTIONNAIRE_SHARED_AXES = {
    "ceiling": _QUESTIONNAIRE_AXIS_PREFERENCES,
    "air_conditioning": _QUESTIONNAIRE_AXIS_PREFERENCES,
    "lighting": _QUESTIONNAIRE_AXIS_PREFERENCES,
}
_QUESTIONNAIRE_AXIS_PRESENTATION = {
    "openness_storage": ("開放感與收納", "continuum", ("open_flow", "寬大走道、視線通透"), ("storage_wall", "整面收納、機能集中")),
    "social_focus": ("交流重心", "continuum", ("media", "影音主導"), ("conversation", "交流主導")),
    "seating_flexibility": ("座位形式", "continuum", ("fixed_sofa", "大型固定沙發"), ("flexible_seating", "分散可移動座位")),
    "sleep_storage": ("睡眠與收納", "continuum", ("calm_sleep", "舒適休息、空間寬鬆"), ("max_storage", "完整衣櫃、收納優先")),
    "work_presence": ("工作機能", "continuum", ("rest_only", "純休息"), ("work_zone", "固定工作區")),
    "bed_access": ("床側動線", "continuum", ("two_side_access", "雙側可上下床"), ("wall_side", "靠牆放大活動區")),
    "table_mode": ("餐桌使用", "continuum", ("compact_daily", "精簡日常"), ("large_gathering", "多人聚餐")),
    "fixed_flexible": ("配置彈性", "continuum", ("fixed", "固定餐區"), ("flexible", "可移動重組")),
    "dining_storage": ("餐區收納", "continuum", ("display", "展示型"), ("closed_storage", "封閉收納型")),
    "kitchen_enclosure": ("開放或封閉", "exclusive", ("open", "開放式廚房"), ("closed", "封閉式廚房")),
    "cooking_intensity": ("料理強度", "continuum", ("light", "簡易料理"), ("heavy", "高頻熱炒")),
    "worktop_storage": ("檯面與收納", "continuum", ("worktop", "大檯面"), ("appliance_storage", "電器收納")),
    "wet_dry": ("乾濕安排", "continuum", ("separated", "完整乾濕分離"), ("open_wet", "開放淋浴區")),
    "bath_mode": ("沐浴方式", "continuum", ("shower", "淋浴優先"), ("tub", "泡澡需求")),
    "bath_storage": ("衛浴收納", "continuum", ("minimal", "檯面留白"), ("full_storage", "完整收納")),
    "balcony_role": ("陽台用途", "continuum", ("utility", "洗曬家務"), ("leisure", "休閒植栽")),
    "balcony_storage": ("陽台收納", "continuum", ("open", "保持通透"), ("storage", "增加收納")),
    "weather_protection": ("氣候防護", "continuum", ("ventilated", "自然通風"), ("protected", "遮雨防曬")),
    "storage_visibility": ("收納可見度", "continuum", ("open", "開放易取"), ("closed", "封閉整齊")),
    "storage_density": ("收納密度", "continuum", ("clearance", "走道優先"), ("capacity", "容量優先")),
    "storage_access": ("取物方式", "continuum", ("daily", "日常快取"), ("archive", "長期備品")),
    "circulation_width": ("通道感受", "continuum", ("wide", "寬敞通行"), ("functional", "牆面機能")),
    "entry_privacy": ("入口視線", "continuum", ("open", "視線通透"), ("screened", "遮擋隱私")),
    "entry_storage": ("入口收納", "continuum", ("light", "輕量落塵"), ("full", "完整玄關收納")),
    "ceiling": ("天花板", "continuum", ("flat", "維持平整、保留淨高"), ("functional_drop", "局部降板、整合管線")),
    "air_conditioning": ("冷氣", "continuum", ("wall_mounted", "壁掛式、路徑直接"), ("concealed", "隱藏式、整合天花")),
    "lighting": ("燈光", "continuum", ("recessed_focus", "偏重崁入式"), ("surface_focus", "偏重明裝式")),
}
_QUESTIONNAIRE_ROOM_OPTIONS = {
    "living_room": {
        "uses": {"日常休息", "親友聚會", "影音娛樂", "親子活動", "居家工作"},
        "furniture": {"沙發", "L 型沙發", "茶几", "電視櫃", "單椅", "收納櫃", "工作桌"},
        "axes": {
            "openness_storage": _QUESTIONNAIRE_AXIS_PREFERENCES,
            "social_focus": _QUESTIONNAIRE_AXIS_PREFERENCES,
            "seating_flexibility": _QUESTIONNAIRE_AXIS_PREFERENCES,
        },
    },
    "bedroom": {
        ...
        "axes": {
            "sleep_storage": _QUESTIONNAIRE_AXIS_PREFERENCES,
            "work_presence": _QUESTIONNAIRE_AXIS_PREFERENCES,
            "bed_access": _QUESTIONNAIRE_AXIS_PREFERENCES,
        },
    },
    "dining_room": {
        ...
        "axes": {
            "table_mode": _QUESTIONNAIRE_AXIS_PREFERENCES,
            "fixed_flexible": _QUESTIONNAIRE_AXIS_PREFERENCES,
            "dining_storage": _QUESTIONNAIRE_AXIS_PREFERENCES,
        },
    },
    "kitchen": {
        ...
        "axes": {
            "kitchen_enclosure": {"a", "b"},
            "cooking_intensity": _QUESTIONNAIRE_AXIS_PREFERENCES,
            "worktop_storage": _QUESTIONNAIRE_AXIS_PREFERENCES,
        },
    },
    "bathroom": {
        ...
        "axes": {
            "wet_dry": _QUESTIONNAIRE_AXIS_PREFERENCES,
            "bath_mode": _QUESTIONNAIRE_AXIS_PREFERENCES,
            "bath_storage": _QUESTIONNAIRE_AXIS_PREFERENCES,
        },
    },
    "balcony": {
        ...
        "axes": {
            "balcony_role": _QUESTIONNAIRE_AXIS_PREFERENCES,
            "balcony_storage": _QUESTIONNAIRE_AXIS_PREFERENCES,
            "weather_protection": _QUESTIONNAIRE_AXIS_PREFERENCES,
        },
    },
    "storage": {
        ...
        "axes": {
            "storage_visibility": _QUESTIONNAIRE_AXIS_PREFERENCES,
            "storage_density": _QUESTIONNAIRE_AXIS_PREFERENCES,
            "storage_access": _QUESTIONNAIRE_AXIS_PREFERENCES,
        },
    },
    "circulation": {
        ...
        "axes": {
            "circulation_width": _QUESTIONNAIRE_AXIS_PREFERENCES,
            "entry_privacy": _QUESTIONNAIRE_AXIS_PREFERENCES,
            "entry_storage": _QUESTIONNAIRE_AXIS_PREFERENCES,
        },
    },
}
_QUESTIONNAIRE_LEGACY_AXIS_PREFERENCES = {
    "open_flow": "a", "storage_wall": "b", "balanced": "balanced",
    "media": "a", "conversation": "b",
    "fixed_sofa": "a", "flexible_seating": "b",
    "calm_sleep": "a", "max_storage": "b", "zoned_storage": "balanced",
    "rest_only": "a", "work_zone": "b",
    "two_side_access": "a", "wall_side": "b",
    "compact_daily": "a", "large_gathering": "b",
    "fixed": "a", "flexible": "b",
    "display": "a", "closed_storage": "b",
    "open": "a", "closed": "b",
    "light": "a", "heavy": "b",
    "worktop": "a", "appliance_storage": "b", "zoned": "balanced",
    "separated": "a", "open_wet": "b",
    "shower": "a", "tub": "b",
    "minimal": "a", "full_storage": "b",
    "utility": "a", "leisure": "b",
    "storage": "b", "ventilated": "a", "protected": "b",
    "clearance": "a", "capacity": "b", "daily": "a", "archive": "b",
    "wide": "a", "functional": "b", "screened": "b", "full": "b",
    "flat": "a", "functional_drop": "b", "zoned_mix": "balanced",
    "wall_mounted": "a", "concealed": "b",
    "recessed_focus": "a", "surface_focus": "b", "layered_by_role": "balanced",
}


def _questionnaire_text(value, limit: int = 4000) -> str:
    return str(value or "").strip()[:limit]


def _questionnaire_strings(value, limit: int = 100) -> list[str]:
    if not isinstance(value, list):
        return []
    return [_questionnaire_text(item, 512) for item in value[:limit] if _questionnaire_text(item, 512)]


def _public_questionnaire_requirements(requirements: dict) -> dict:
    return {
...
def _sanitize_invited_basic(value) -> dict:
    raw = value if isinstance(value, dict) else {}
    cleaned = {}
    for key in _QUESTIONNAIRE_BASIC_KEYS:
        item = raw.get(key)
        allowed = _QUESTIONNAIRE_BASIC_OPTIONS[key]
        if key in _QUESTIONNAIRE_MULTI_BASIC_KEYS:
            source = item if isinstance(item, list) else [item]
            selected = [choice for choice in _questionnaire_strings(source) if choice in allowed]
            exclusive = _QUESTIONNAIRE_BASIC_EXCLUSIVE.get(key, set())
            if len(selected) > 1 and any(choice in exclusive for choice in selected):
                selected = [choice for choice in selected if choice not in exclusive]
            cleaned[key] = list(dict.fromkeys(selected))
        else:
            source = item if isinstance(item, list) else [item]
            selected = next(
                (choice for choice in _questionnaire_strings(source) if choice in allowed),
                "",
            )
            cleaned[key] = selected if selected in allowed else ""
    notes = raw.get("notes") if isinstance(raw.get("notes"), dict) else {}
    cleaned["notes"] = {
        key: _questionnaire_text(notes.get(key))
        for key in _QUESTIONNAIRE_BASIC_KEYS
        if notes.get(key)
    }
    return cleaned
...
def _canonical_questionnaire_room_type(value) -> str:
    room_type = str(value or "").lower()
    return {
        "dormitory": "bedroom",
        ...
        "hallway": "circulation",
    }.get(room_type, room_type if room_type in _QUESTIONNAIRE_ROOM_OPTIONS else "storage")


def _questionnaire_axis_options(room_type: str) -> dict[str, set[str]]:
    canonical_room_type = _canonical_questionnaire_room_type(room_type)
    options = _QUESTIONNAIRE_ROOM_OPTIONS[canonical_room_type]
    shared_axes = {
        "ceiling": _QUESTIONNAIRE_SHARED_AXES["ceiling"],
    }
    if canonical_room_type in {"living_room", "bedroom", "dining_room"}:
        shared_axes["air_conditioning"] = _QUESTIONNAIRE_SHARED_AXES["air_conditioning"]
    shared_axes["lighting"] = _QUESTIONNAIRE_SHARED_AXES["lighting"]
    return {**options["axes"], **shared_axes}


def _questionnaire_basic_is_complete(basic: dict) -> bool:
    return all(
        bool(basic.get(key))
        for key in _QUESTIONNAIRE_BASIC_KEYS
    )
...
def _questionnaire_room_validation_error(
    room_type: str,
    answer: dict,
    *,
    room_height_cm: float,
    minimum_finished_height_cm: float,
) -> str:
    canonical_room_type = _canonical_questionnaire_room_type(room_type)
    if not answer.get("uses"):
        return "questionnaire_room_use_required"
    axis_options = _questionnaire_axis_options(canonical_room_type)
    axes = answer.get("axes") if isinstance(answer.get("axes"), dict) else {}
    if any(axes.get(axis_id) not in allowed for axis_id, allowed in axis_options.items()):
        return "questionnaire_room_axes_incomplete"
    ceiling_drop_cm = {
        "a": 0,
        "lean_a": 8,
        "balanced": 15,
    ...
    if float(room_height_cm) - ceiling_drop_cm < float(minimum_finished_height_cm):
        return "minimum_finished_height_not_met"
    return ""


def _merge_questionnaire_validation_state(base: dict, patch: dict) -> dict:
    merged = dict(base)
    for key, value in patch.items():
        if isinstance(value, dict) and isinstance(merged.get(key), dict):
            merged[key] = _merge_questionnaire_validation_state(merged[key], value)
        else:
            merged[key] = value
    return merged


def _validate_confirmed_questionnaire_workflow(workflow: dict) -> None:
    requirements = workflow.get("requirements")
    if not isinstance(requirements, dict):
        return
    sanitized_basic = _sanitize_invited_basic(requirements.get("basic"))
    if (
        requirements.get("basicConfirmed") is True
        and not _questionnaire_basic_is_complete(sanitized_basic)
    ):
        raise HTTPException(
            422,
            {
                "code": "questionnaire_basic_incomplete",
                "message": "基本問卷仍有必填題未完成。",
            },
        )

    rooms = workflow.get("space_confirmation", {}).get("rooms", [])
    room_by_id = {
        str(room.get("id") or ""): room
        for room in rooms
        if isinstance(room, dict) and room.get("id")
    }
    answers = requirements.get("rooms")
    if not isinstance(answers, dict) or not room_by_id:
        return
    settings = requirements.get("settings") or {}
    minimum_finished_height_cm = min(
        300,
        max(210, float(settings.get("minimumFinishedHeightCm") or 240)),
    )
    confirmed_floorplan = workflow.get("confirmed_floorplan") or {}
    confirmed_floorplan = confirmed_floorplan.get("floorplan") or {}
    room_height_cm = float(confirmed_floorplan.get("room_height_cm") or 270)
    for room_id, answer in answers.items():
        room = room_by_id.get(str(room_id))
        if not room or not isinstance(answer, dict) or answer.get("confirmed") is not True:
            continue
        sanitized_answer = _sanitize_invited_room_answer(
            answer,
            room_type=room.get("type"),
        )
        validation_error = _questionnaire_room_validation_error(
            room.get("type"),
            sanitized_answer,
            room_height_cm=room_height_cm,
            minimum_finished_height_cm=minimum_finished_height_cm,
        )
        if validation_error:
            raise HTTPException(
                422,
...
def _questionnaire_axis_detail(
    room_type: str,
    axis_id: str,
    preference: str,
    other_approach: str,
) -> dict:
    label, mode, endpoint_a, endpoint_b = _QUESTIONNAIRE_AXIS_PRESENTATION[axis_id]
    room_slug = _canonical_questionnaire_room_type(room_type).replace("_", "-")
    axis_slug = axis_id.replace("_", "-")

    def endpoint_payload(endpoint: tuple[str, str]) -> dict:
        value, endpoint_label = endpoint
        return {
            "value": value,
            "label": endpoint_label,
            "image_key": f"{room_slug}/axis/{axis_slug}/{value.replace('_', '-')}",
        }

    selected_label = {
        "a": endpoint_a[1],
        "lean_a": f"偏重 A：{endpoint_a[1]}",
        "balanced": "兩者平衡",
        "lean_b": f"偏重 B：{endpoint_b[1]}",
        "b": endpoint_b[1],
        "": "",
    }[preference]
    return {
        ...
        "mode": mode,
        "axis_label": label,
        "endpoint_a": endpoint_payload(endpoint_a),
        "endpoint_b": endpoint_payload(endpoint_b),
        "other_approach": other_approach,
    }


def _sanitize_invited_room_answer(
    value,
    ...
    *,
    ...
    custom_notes = raw.get("customNotes") if isinstance(raw.get("customNotes"), dict) else {}
    stage_notes = raw.get("stageNotes") if isinstance(raw.get("stageNotes"), dict) else {}
    cleaned.update(
        {
            "schemaVersion": "3.0",
            "confirmed": False,
            "keepExisting": raw.get("keepExisting") is True,
            "axes": {
                key: (
                    _QUESTIONNAIRE_LEGACY_AXIS_PREFERENCES.get(
                        _questionnaire_text(item, 256),
                        _questionnaire_text(item, 256),
                    )
                )
                ...
                if (
                    key in axis_options
                    and _QUESTIONNAIRE_LEGACY_AXIS_PREFERENCES.get(
                        _questionnaire_text(item, 256),
                        _questionnaire_text(item, 256),
                    ) in axis_options[key]
                )
            ...
            "customNotes": {
                key: _questionnaire_text(item)
                for key, item in custom_notes.items()
                if key in axis_options and _questionnaire_text(item)
            },
            "stageNotes": {
                key: _questionnaire_text(stage_notes.get(key))
                for key in ("uses", "furniture")
                if _questionnaire_text(stage_notes.get(key))
            },
            "uses": [
                item for item in _questionnaire_strings(raw.get("uses"))
                if item in options["uses"]
            ],
            "furniture": [
                item for item in _questionnaire_strings(raw.get("furniture"))
                if item in options["furniture"]
            ],
            "personalNeeds": _questionnaire_text(raw.get("personalNeeds")),
        }
    )
    identity = raw.get("roomIdentity")
    if isinstance(identity, dict):
        centroid = identity.get("centroid_m")
        try:
            center_x = float(centroid.get("x")) if isinstance(centroid, dict) else None
            center_y = float(centroid.get("y")) if isinstance(centroid, dict) else None
            area_m2 = float(identity.get("area_m2"))
        except (TypeError, ValueError):
            center_x = center_y = area_m2 = None
        if (
            ...
            and -10_000 <= center_x <= 10_000
            and -10_000 <= center_y <= 10_000
            and 0 <= area_m2 <= 1_000_000
        ):
            cleaned["roomIdentity"] = {
                "type": _canonical_questionnaire_room_type(room_type),
                "label": _questionnaire_text(identity.get("label"), 128),
                "centroid_m": {"x": center_x, "y": center_y},
                "area_m2": area_m2,
            }
    preferences = raw.get("materialPreferences")
    if isinstance(preferences, dict):
        cleaned["materialPreferences"] = {
            "wall": [
                item for item in _questionnaire_strings(preferences.get("wall"))
                if item in {
                    "paint", "mineral", "microcement", "stone",
                    "wood", "tile", "fabric", "metal",
                }
            ],
            "floor": [
                item for item in _questionnaire_strings(preferences.get("floor"))
                if item in {"wood", "tile", "stone", "cement", "terrazzo", "vinyl"}
            ],
            "furniture": [
                item for item in _questionnaire_strings(preferences.get("furniture"))
                if item in {"wood", "fabric", "leather", "metal", "glass", "stone"}
            ],
            "color": [
                item for item in _questionnaire_strings(preferences.get("color"))
                if item in {
                    "warm_neutral", "cool_neutral", "earth",
                    "green", "dark", "high_contrast",
                }
            ],
            "finish": [
                item for item in _questionnaire_strings(preferences.get("finish"))
                if item in {"matte", "eggshell", "semi_gloss", "gloss", "textured"}
            ],
            "cuts": _questionnaire_strings(preferences.get("cuts")),
        }
    return {key: item for key, item in cleaned.items() if key in _QUESTIONNAIRE_ROOM_KEYS}


def _questionnaire_risks(room: dict, answer: dict) -> list[str]:
    room_type = _canonical_questionnaire_room_type(room.get("type"))
    axes = answer.get("axes") if isinstance(answer.get("axes"), dict) else {}
    axes = {
        key: _QUESTIONNAIRE_LEGACY_AXIS_PREFERENCES.get(str(value), str(value))
        for key, value in axes.items()
    }
    risks = []
    if room_type == "kitchen" and axes.get("kitchen_enclosure") == "a":
        risks.extend(["wall", "gas", "smoke_exhaust"])
    if room_type == "kitchen" and axes.get("cooking_intensity") in {
        "lean_a", "balanced", "lean_b", "b",
    }:
        risks.extend(["gas", "smoke_exhaust"])
    if room_type == "bathroom" and axes.get("bath_mode") in {
        "lean_a", "balanced", "lean_b", "b",
    }:
        risks.extend(["plumbing", "drainage"])
    if room_type == "balcony" and axes.get("balcony_role") in {
        "a", "lean_a", "balanced", "lean_b",
    }:
        risks.extend(["plumbing", "drainage"])
    if axes.get("air_conditioning") in {"lean_a", "balanced", "lean_b", "b"}:
        risks.append("electricity")
    free_text = " ".join(
        [
            *[str(item) for item in (answer.get("customNotes") or {}).values()],
            *[str(item) for item in (answer.get("stageNotes") or {}).values()],
            str(answer.get("priority") or ""),
            str(answer.get("personalNeeds") or ""),
            *[str(item) for item in (answer.get("materialPreferences") or {}).get("cuts", [])],
        ]
    )
    keyword_risks = {
        "wall": ("拆牆", "改牆", "移牆", "打牆", "開放式"),
        "gas": ("瓦斯", "燃氣"),
        "electricity": ("插座", "電路", "用電", "220v", "電箱", "電力", "新增燈"),
    ...
    for risk, keywords in keyword_risks.items():
        if any(keyword in lowered for keyword in keywords):
            risks.append(risk)
    return list(dict.fromkeys(risks))


def _build_invited_client_brief(rooms: list[dict], requirements: dict) -> dict:
    basic = requirements.get("basic") or {}
    answers = requirements.get("rooms") or {}
    keep_existing = set(requirements.get("keepExistingRoomIds") or [])
    room_briefs = {}
    warnings = []
    risk_messages = {
    ...
    for room in rooms:
        ...
        answer = answers.get(room_id) if isinstance(answers.get(room_id), dict) else {}
        preferences = answer.get("materialPreferences") or {}
        material_preferences = {
            "status": "defined" if any(
                preferences.get(key)
                for key in ("wall", "floor", "furniture", "color", "finish", "cuts")
            ) else "not_defined",
            "wall": preferences.get("wall") or [],
            "floor": preferences.get("floor") or [],
            "furniture": preferences.get("furniture") or [],
            "color": preferences.get("color") or [],
            "finish": preferences.get("finish") or [],
            "cuts": preferences.get("cuts") or [],
        }
        if room_id in keep_existing:
            room_briefs[room_id] = {
                "room_id": room_id,
            ...
            continue
        room_risks = _questionnaire_risks(room, answer)
        axis_values = answer.get("axes") if isinstance(answer.get("axes"), dict) else {}
        axis_notes = (
            answer.get("customNotes")
            if isinstance(answer.get("customNotes"), dict)
            else {}
        )
        preference_axis_details = {}
        decisions = []
        for axis_id in _questionnaire_axis_options(room.get("type")):
            preference = _QUESTIONNAIRE_LEGACY_AXIS_PREFERENCES.get(
                str(axis_values.get(axis_id) or ""),
                str(axis_values.get(axis_id) or ""),
            )
            if preference not in _QUESTIONNAIRE_AXIS_PREFERENCES:
                preference = ""
            detail = _questionnaire_axis_detail(
                room.get("type"),
                axis_id,
                preference,
                axis_notes.get(axis_id, ""),
            )
            preference_axis_details[axis_id] = {
                key: detail[key]
                for key in (
                    "preference",
                    "selected_label",
            ...
            if not preference:
                continue
            explanation = {
                "a": f"明確採用 A「{detail['endpoint_a']['label']}」",
                "lean_a": (
                    f"以 A「{detail['endpoint_a']['label']}」為主，"
                    "保留少量 B 的機能"
                ),
                "balanced": (
                    f"在 A「{detail['endpoint_a']['label']}」與 "
                    f"B「{detail['endpoint_b']['label']}」之間平衡"
                ),
                "lean_b": (
                    f"以 B「{detail['endpoint_b']['label']}」為主，"
                    "保留少量 A 的特點"
                ),
                "b": f"明確採用 B「{detail['endpoint_b']['label']}」",
            }[preference]
            decisions.append(
                {
                    "axis_id": axis_id,
                    "axis_label": detail["axis_label"],
                    "preference": preference,
                    "selected_label": detail["selected_label"],
                    "basis": explanation,
                    "explanation": explanation,
                    "other_approach": detail["other_approach"],
                }
            )
        room_warning_items = [
            {
                "id": f"{room_id}:{risk}",
                "roomId": room_id,
                "roomLabel": room.get("label"),
                "risk": risk,
                "reason": risk_messages[risk],
                "index": index,
                "total": len(room_risks),
                "position": f"{index}/{len(room_risks)}",
            }
            for index, risk in enumerate(room_risks, start=1)
        ]
        summary_basis = [
            (
                f"使用需求：{'、'.join(answer.get('uses') or [])}"
                if answer.get("uses")
                else ""
            ),
            *[
                f"{decision['axis_label']}：{decision['explanation']}"
        ...
        room_briefs[room_id] = {
            ...
            "room_label": room.get("label"),
            "room_type": room.get("type"),
            "planning_status": "confirmed" if answer.get("confirmed") is True else "incomplete",
            "uses": answer.get("uses") or [],
            "furniture_requirements": answer.get("furniture") or [],
            "preference_axes": answer.get("axes") or {},
            "preference_axis_details": preference_axis_details,
            "integrated_summary": {
                "room_id": room_id,
                "room_label": room.get("label"),
                "headline": (
                    f"{room.get('label') or '此空間'}已整理 {len(decisions)} 項偏好，"
                    "可交給家具與材質檢索。"
                    if decisions
                    else f"尚未完成{room.get('label') or '此空間'}的偏好選擇。"
                ),
                "basis": [item for item in summary_basis if item],
                "decisions": decisions,
                "furniture": answer.get("furniture") or [],
                "other_approaches": [
                    f"{detail['axis_label']}：{detail['other_approach']}"
                    for detail in (
                        _questionnaire_axis_detail(
                            room.get("type"),
                            axis_id,
                            axis_detail["preference"],
                            axis_detail["other_approach"],
                        )
                        for axis_id, axis_detail in preference_axis_details.items()
                    )
                    if detail["other_approach"]
                ],
                "warnings": room_warning_items,
            },
            "custom_notes": answer.get("customNotes") or {},
            "stage_notes": answer.get("stageNotes") or {
                "uses": "",
                "furniture": "",
            },
            "priority": answer.get("priority") or "",
            "personal_needs": answer.get("personalNeeds") or "無",
            "material_preferences": material_preferences,
            "structure_strategy": "compare_changed_and_unchanged"
            if "wall" in room_risks
            else "keep_current_structure",
            "safety_risks": room_risks,
        }
    ...
    warnings = [
        {
            **warning,
            "index": index,
            "total": warning_total,
            "position": f"{index}/{warning_total}",
        }
        for index, warning in enumerate(warnings, start=1)
    ]
    return {
        "schema_version": requirements.get("schemaVersion") or "3.0",
        "occupants": {
            "residents": basic.get("residents") or [],
            "resident_count": basic.get("residentCount") or "",
            "age_needs": basic.get("ageNeeds") or [],
        },
        "lifestyle": {
            "schedule_interference": basic.get("scheduleInterference") or [],
            "home_work_study": {
                "frequency_and_people": basic.get("homeWorkStudyCount") or "",
                "needs": basic.get("homeWorkStudyNeeds") or [],
            },
            "hosting": {
                "frequency": basic.get("hostingFrequency") or "",
                "needs": basic.get("hostingNeeds") or [],
            },
        },
        "future_changes": basic.get("futureChanges") or [],
        "budget_and_timeline": {
            "priority": basic.get("budgetPriority") or "",
            "range": basic.get("budgetRange") or "",
            "target": basic.get("targetTimeline") or "",
        },
        "immutable_needs": basic.get("immutableNeeds") or [],
        "basic_notes": basic.get("notes") or {},
        "rooms": room_briefs,
        "warnings": warnings,
        "designer_notes": requirements.get("designerNotes") or "",
        "privacy": {"project_only": True, "no_training": True},
    }
...
@app.post("/api/projects/{project_id}/questionnaire-invite", status_code=201)
def create_questionnaire_invite(project_id: str) -> dict:
    _stored_project(project_id)
    invitation = PROJECT_STORE.create_questionnaire_invite(project_id)
    return {
        **invitation,
        "questionnaire_url": f"/questionnaire/{invitation['invite_token']}",
    }


@app.delete("/api/projects/{project_id}/questionnaire-invite/{invite_token}")
def revoke_questionnaire_invite(project_id: str, invite_token: str) -> dict:
    _stored_project(project_id)
    try:
        PROJECT_STORE.revoke_questionnaire_invite(project_id, invite_token)
    except KeyError as exc:
        raise HTTPException(404, "questionnaire_invite_not_found") from exc
    return {"revoked": True}


@app.delete("/api/projects/{project_id}/questionnaire-invites")
def revoke_all_questionnaire_invites(project_id: str) -> dict:
    _stored_project(project_id)
    return {
        "revoked": PROJECT_STORE.revoke_all_questionnaire_invites(project_id),
    }


def _questionnaire_floorplan_locator(recognition: dict) -> dict:
    image_size = recognition.get("image_size_px")
    plan_bbox = recognition.get("plan_bbox_px")
    scale = recognition.get("scale")
    floorplan = recognition.get("floorplan") or {}
    public_scale = dict(scale or {})
    if public_scale.get("distance_cm") is None and public_scale.get("distance_m") is not None:
        public_scale["distance_cm"] = float(public_scale["distance_m"]) * 100
    if public_scale.get("cm_per_px") is None and public_scale.get("m_per_px") is not None:
        public_scale["cm_per_px"] = float(public_scale["m_per_px"]) * 100
    public_scale.pop("distance_m", None)
    public_scale.pop("m_per_px", None)
    if image_size and plan_bbox and scale:
        return {
            "image_size_px": image_size,
            "plan_bbox_px": plan_bbox,
            "scale": public_scale,
    ...
    width_cm = max(float(floorplan.get("width_cm") or 600), 1)
    depth_cm = max(float(floorplan.get("depth_cm") or 400), 1)
    preview_width = 1000
    preview_height = max(1, round(preview_width * depth_cm / width_cm))
    return {
        "image_size_px": {"width": preview_width, "height": preview_height},
        "plan_bbox_px": [0, 0, preview_width, preview_height],
        "coordinate_space": "lower_left_cm",
        "scale": {
            "distance_cm": width_cm,
            "cm_per_px": width_cm / preview_width,
            "source": "dxf_geometry",
        },
    }


def _questionnaire_room_polygon_cm(room: dict) -> list[dict]:
    polygon = room.get("polygon_cm")
    multiplier = 1
    if not polygon:
        polygon = room.get("polygon_m")
        multiplier = 100
    polygon_cm: list[dict] = []
    for point in polygon or []:
        if isinstance(point, dict):
            polygon_cm.append(
                {
                    "x": float(point.get("x") or 0) * multiplier,
                    "y": float(point.get("y", point.get("z", 0)) or 0) * multiplier,
                }
            )
        elif isinstance(point, (list, tuple)) and len(point) >= 2:
            polygon_cm.append(
                {
                    "x": float(point[0]) * multiplier,
                    "y": float(point[1]) * multiplier,
                }
            )
    return polygon_cm


def _questionnaire_dxf_preview(floorplan: dict) -> str:
    width_m = max(float(floorplan.get("width_cm") or 600) / 100, 0.01)
    depth_m = max(float(floorplan.get("depth_cm") or 400) / 100, 0.01)
    pixel_width = 1000
    pixel_height = max(1, round(pixel_width * depth_m / width_m))
    lines: list[str] = []
    for segment in floorplan.get("wall_segments") or floorplan.get("plan_segments") or []:
        start = segment.get("start") if isinstance(segment, dict) else segment[0]
        end = segment.get("end") if isinstance(segment, dict) else segment[1]
        if isinstance(start, dict):
            start_x = float(start.get("x") or 0)
            start_z = float(start.get("z", start.get("y", 0)) or 0)
        else:
            start_x, start_z = float(start[0]), float(start[1])
        if isinstance(end, dict):
            end_x = float(end.get("x") or 0)
            end_z = float(end.get("z", end.get("y", 0)) or 0)
        else:
            end_x, end_z = float(end[0]), float(end[1])
        lines.append(
            '<line '
            f'x1="{start_x + width_m / 2:.6f}" '
            f'y1="{depth_m / 2 - start_z:.6f}" '
            f'x2="{end_x + width_m / 2:.6f}" '
            f'y2="{depth_m / 2 - end_z:.6f}"/>'
        )
    stroke_width = max(width_m, depth_m) / 500
    return (
        f'<svg xmlns="http://www.w3.org/2000/svg" width="{pixel_width}" '
        f'height="{pixel_height}" viewBox="0 0 {width_m:.6f} {depth_m:.6f}">'
        f'<rect width="{width_m:.6f}" height="{depth_m:.6f}" fill="white"/>'
        f'<g stroke="#222" stroke-width="{stroke_width:.6f}">'
        f'{"".join(lines)}</g></svg>'
    )
...
@app.get("/api/questionnaire/{invite_token}")
def get_invited_questionnaire(invite_token: str, response: Response) -> dict:
    project = _questionnaire_project(invite_token)
    workflow = project.get("workflow", {})
    rooms = workflow.get("space_confirmation", {}).get("rooms", [])
    recognition = workflow.get("recognition") or {}
    requirements = workflow.get("requirements") or {}
    response.headers.update(_questionnaire_private_headers())
    locator = _questionnaire_floorplan_locator(recognition)
    confirmed_floorplan = workflow.get("confirmed_floorplan") or {}
    confirmed_floorplan = confirmed_floorplan.get("floorplan") or {}
    return {
        "project": {
        ...
        "floorplan": {
            "source_url": f"/api/questionnaire/{invite_token}/floorplan",
            "room_height_cm": confirmed_floorplan.get("room_height_cm", 270),
            **locator,
        },
        "rooms": [
            {
                ...
                "type": room.get("type"),
                **(
                    {"polygon_cm": _questionnaire_room_polygon_cm(room)}
                    if room.get("polygon_cm") or room.get("polygon_m")
                    else {}
                ),
            }
            for room in rooms
        ],
        "requirements": _public_questionnaire_requirements(requirements),
    }


@app.get("/api/questionnaire/{invite_token}/floorplan")
def get_invited_questionnaire_floorplan(invite_token: str) -> Response:
    project = _questionnaire_project(invite_token)
    upload = _stored_floorplan(project["project_id"])
    if upload["extension"] == ".dxf":
        recognition = project.get("workflow", {}).get("recognition") or {}
        floorplan = recognition.get("floorplan") or {}
        return Response(
            _questionnaire_dxf_preview(floorplan),
            media_type="image/svg+xml",
            headers=_questionnaire_private_headers(),
        )
    return FileResponse(
        upload["path"],
...
@app.put("/api/questionnaire/{invite_token}")
def save_invited_questionnaire(
    invite_token: str,
    payload: dict,
    response: Response,
) -> dict:
    project = _questionnaire_project(invite_token)
    response.headers.update(_questionnaire_private_headers())
    incoming = payload.get("requirements")
    if not isinstance(incoming, dict):
        raise HTTPException(422, "questionnaire_requirements_required")
    base_updated_at = _questionnaire_text(payload.get("base_updated_at"), 128)
    if not base_updated_at:
        raise HTTPException(422, "questionnaire_base_version_required")
    workflow = project.get("workflow") or {}
    rooms = workflow.get("space_confirmation", {}).get("rooms", [])
    allowed_room_ids = {str(room.get("id") or "") for room in rooms}
    room_by_id = {str(room.get("id") or ""): room for room in rooms}
    current_requirements = workflow.get("requirements") or {}
    current_answers = current_requirements.get("rooms") or {}
    incoming_answers = incoming.get("rooms") if isinstance(incoming.get("rooms"), dict) else {}
    settings = current_requirements.get("settings") or {
    ...
    confirmed_floorplan = confirmed_floorplan.get("floorplan") or {}
    room_height_cm = float(confirmed_floorplan.get("room_height_cm") or 270)
    sanitized_basic = _sanitize_invited_basic(incoming.get("basic"))
    if (
        incoming.get("basicConfirmed") is True
        and not _questionnaire_basic_is_complete(sanitized_basic)
    ):
        raise HTTPException(
            422,
    ...
    next_answers = dict(current_answers)
    for room_id, answer in incoming_answers.items():
        room_id = str(room_id)
        if room_id not in allowed_room_ids or not isinstance(answer, dict):
            continue
        sanitized_answer = _sanitize_invited_room_answer(
            answer,
            current_answers.get(room_id) if isinstance(current_answers.get(room_id), dict) else None,
            room_type=room_by_id[room_id].get("type"),
        )
        if answer.get("confirmed") is True:
            validation_error = _questionnaire_room_validation_error(
                room_by_id[room_id].get("type"),
                sanitized_answer,
                room_height_cm=room_height_cm,
                minimum_finished_height_cm=minimum_finished_height_cm,
            )
            if validation_error:
                raise HTTPException(
                    422,
                    {
                        ...
                        "room_id": room_id,
                        "message": (
                            "房間需求未完成，或天花方案低於設計師設定的最低完成淨高。"
                        ),
                    },
                )
            sanitized_answer["confirmed"] = True
        next_answers[room_id] = sanitized_answer
    next_requirements = {
        **current_requirements,
        "schemaVersion": "3.0",
        "basic": sanitized_basic,
        "basicConfirmed": (
            incoming.get("basicConfirmed") is True
            and _questionnaire_basic_is_complete(sanitized_basic)
        ),
        "rooms": next_answers,
        "keepExistingRoomIds": [
            room_id
            for room_id in _questionnaire_strings(incoming.get("keepExistingRoomIds"))
            if room_id in allowed_room_ids
        ],
        "mode": "client_invite",
        "settings": {
            **settings,
            "minimumFinishedHeightCm": minimum_finished_height_cm,
        },
    }
    next_requirements["clientBrief"] = _build_invited_client_brief(rooms, next_requirements)
    try:
        updated = PROJECT_STORE.update_workflow(
            project["project_id"],
            current_step=project["current_step"],
            workflow={"requirements": next_requirements},
...
def save_project_workflow(project_id: str, payload: dict) -> dict:
    ...
    merged_workflow = _merge_questionnaire_validation_state(
        stored_project.get("workflow") or {},
        workflow or {},
    )
    if isinstance((workflow or {}).get("requirements"), dict):
        _validate_confirmed_questionnaire_workflow(merged_workflow)
    _validate_confirmed_material_workflow(merged_workflow)
    expected_updated_at = str(payload.get("base_updated_at") or "").strip() or None
    if payload.get("replay_pending") is True and not expected_updated_at:
...
async def generate_scene(payload: dict) -> dict:
    ...
    questionnaire = {
        ...
        "wall_color_hex": payload.get("wall_color_hex"),
        "floor_color_hex": payload.get("floor_color_hex"),
