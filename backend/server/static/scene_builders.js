...
// 場景建構共用模組:scene_viewer(編輯視角)與 panorama(環景漫遊)共用。
// 材質函式都吃 renderer 是為了 anisotropy 上限。

export function makeCanvasTexture(renderer, { width = 1024, height = 1024, repeatX = 1, repeatY = 1, draw }) {
...
function createWoodTexture(renderer, base, grain, seam, repeatX, repeatY) {
  return makeCanvasTexture(renderer, {
    repeatX,
    repeatY,
    draw(context, width, height) {
...
function createHerringboneTexture(renderer, base, grain, seam, repeatX, repeatY) {
  return makeCanvasTexture(renderer, {
    repeatX,
    repeatY,
    draw(context, width, height) {
...
function createStoneTexture(renderer, base, vein, seam, repeatX, repeatY) {
  return makeCanvasTexture(renderer, {
    repeatX,
    repeatY,
    draw(context, width, height) {
...
function createMarbleTexture(renderer, base, vein, accent, seam, repeatX, repeatY) {
  return makeCanvasTexture(renderer, {
    repeatX,
    repeatY,
    draw(context, width, height) {
...
function createMicrocementTexture(renderer, base, cloud, repeatX, repeatY) {
  return makeCanvasTexture(renderer, {
    repeatX,
    repeatY,
    draw(context, width, height) {
...
function createWallTexture(renderer, base, accent, repeatX, repeatY) {
  return makeCanvasTexture(renderer, {
    repeatX,
    repeatY,
    draw(context, width, height) {
...
export function createFloorMaterial(renderer, floorOption) {
  const presets = {
    auto: () =>
      new THREE.MeshStandardMaterial({
        color: 0xe6d1ae,
        map: createWoodTexture(renderer, "#e3c99f", "#b48d63", "#cfb288", 4, 4),
        roughness: 0.9,
        metalness: 0.01,
      }),
    light_oak: () =>
      new THREE.MeshStandardMaterial({
        color: 0xe0c18d,
        map: createWoodTexture(renderer, "#e7cca1", "#be9871", "#d7b58c", 4, 4),
        roughness: 0.88,
        metalness: 0.01,
      }),
    herringbone_oak: () =>
      new THREE.MeshStandardMaterial({
        color: 0xe4c395,
        map: createHerringboneTexture(renderer, "#e9cda3", "#ad8054", "#d0ad82", 3.4, 3.4),
        roughness: 0.82,
        metalness: 0.01,
      }),
    walnut: () =>
      new THREE.MeshStandardMaterial({
        color: 0x6f4d34,
        map: createWoodTexture(renderer, "#7b563a", "#4d3221", "#8f694a", 4, 4),
        roughness: 0.84,
        metalness: 0.02,
      }),
    stone_gray: () =>
      new THREE.MeshStandardMaterial({
        color: 0xd7d3cf,
        map: createStoneTexture(renderer, "#d6d2cd", "#bdb7b2", "#f0ece7", 3, 3),
        roughness: 0.92,
        metalness: 0.01,
      }),
    marble: () =>
      new THREE.MeshStandardMaterial({
        color: 0xf2eee7,
        map: createMarbleTexture(renderer, "#f8f5ef", "#9d968d", "#c6aa82", "#efe7dc", 2.2, 2.2),
        roughness: 0.38,
        metalness: 0.02,
      }),
    microcement: () =>
      new THREE.MeshStandardMaterial({
        color: 0xbcb3aa,
        map: createMicrocementTexture(renderer, "#beb4aa", "#948a80", 3, 3),
        roughness: 0.96,
        metalness: 0.01,
      }),
  };
...
export function createWallMaterial(renderer, wallOption) {
  const presets = {
    auto: () =>
      new THREE.MeshStandardMaterial({
        color: 0xf7f2eb,
        map: createWallTexture(renderer, "#f5efe7", "#d8cebf", 2.2, 1.6),
        roughness: 0.98,
        metalness: 0.01,
    ...
    warm_white: () =>
      new THREE.MeshStandardMaterial({
        color: 0xf8f3eb,
        map: createWallTexture(renderer, "#f8f2ea", "#ddd3c4", 2.2, 1.6),
        roughness: 0.97,
        metalness: 0.01,
        side: THREE.DoubleSide,
      }),
    mineral_beige: () =>
      new THREE.MeshStandardMaterial({
        color: 0xd2bea6,
        map: createWallTexture(renderer, "#d0baa0", "#b59a7d", 2.1, 1.5),
        roughness: 0.99,
        metalness: 0.01,
        side: THREE.DoubleSide,
      }),
    light_gray: () =>
      new THREE.MeshStandardMaterial({
        color: 0xe0e2e5,
        map: createWallTexture(renderer, "#dfe2e5", "#c7cbd1", 2.2, 1.6),
        roughness: 0.98,
        metalness: 0.01,
        side: THREE.DoubleSide,
      }),
    limewash: () =>
      new THREE.MeshStandardMaterial({
        color: 0xded1bd,
        map: createWallTexture(renderer, "#e6dac8", "#bfa98e", 2.4, 1.7),
        roughness: 1,
        metalness: 0,
        side: THREE.DoubleSide,
      }),
    charcoal: () =>
      new THREE.MeshStandardMaterial({
        color: 0x5a5550,
        map: createWallTexture(renderer, "#59544f", "#77706a", 2.2, 1.6),
        roughness: 0.94,
        metalness: 0.02,
        side: THREE.DoubleSide,
      }),
  ...
  return (presets[wallOption] ?? presets.auto)();
}

export function buildSegmentWalls(group, segments, wallMaterial, wallHeight, wallThickness, register) {
  segments.forEach((segment) => {
    ...
    group.add(register ? register(wallMesh) : wallMesh);
  });
}

export function buildFloorPlanOverlay(group, segments, color, opacity = 0.55, yOffset = 0.025) {
...
export function buildWindowBoxes(group, segments, wallHeight) {
  // 立體窗使用半透明藍盒。平面圖沒有離地高度資訊，
...
export function moveModelToFootprintCenter(root) {
...
export function fitToTargetSize(root, targetSizeCm) {
  root.updateMatrixWorld(true);
  const box = new THREE.Box3().setFromObject(root);
