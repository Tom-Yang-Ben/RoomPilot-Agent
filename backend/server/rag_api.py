...
RAG_JOB_MAX_QUEUED = 24
RAG_JOB_QUEUE: Queue[tuple[str, RagSearchRequest]] = Queue(maxsize=RAG_JOB_MAX_QUEUED)
RAG_JOB_WORKER_STARTED = False
RAG_JOB_WORKER_LOCK = Lock()
...
def _run_job_worker() -> None:
    """Serialize local embedding work without rejecting later room confirmations."""
    while True:
        job_id, request = RAG_JOB_QUEUE.get()
        try:
            _run_job(job_id, request)
        finally:
            RAG_JOB_QUEUE.task_done()


def _ensure_job_worker() -> None:
    global RAG_JOB_WORKER_STARTED
    with RAG_JOB_WORKER_LOCK:
        if RAG_JOB_WORKER_STARTED:
            return
        Thread(target=_run_job_worker, daemon=True).start()
        RAG_JOB_WORKER_STARTED = True
...
def create_rag_search_job(request: RagSearchRequest) -> dict:
    ...
    with RAG_JOBS_LOCK:
        ...
        if active_jobs >= RAG_JOB_MAX_QUEUED:
    ...
    _ensure_job_worker()
    RAG_JOB_QUEUE.put((job_id, request))
    return snapshot
