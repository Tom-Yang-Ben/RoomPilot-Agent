...
from ...agent import placement_hints, resolve_placements
...
DOTENV_CANDIDATES = [
    ...
    # 2026-07-22 重組前的舊位置:目錄 rename 不會搬未追蹤的本機 .env,
    # 留這條後備讓只把 key 放在舊路徑的組員不會靜默失去 LLM 功能。
...
def _post_openrouter_chat(
    payload: dict[str, Any], headers: dict[str, str], *, timeout: float = 8
) -> dict[str, Any] | None:
    req = request.Request(
...
def build_scene_payload(
    ...
    # agent 擺位紀律:潛規則確定性提示(主件先、副件成組最後,不出座標)只影響
    # 引擎試放順序,合法性仍由 check_placement_with_clearance 把關;放不下 →
    # 寧缺勿亂(副件退場)/ 換更小同型 / 移除,重擺至收斂。座標 100% 由引擎算。
    ...
    objects = _place(selected_items)
    objects, selected_items, recovery = resolve_placements(
        objects,
        selected_items,
        site_payload["furniture"],
        place_fn=_place,
