...
def save_vis(name, ans_bgr, gt, pr):
    ...
    out = os.path.join("color_chk", f"eval_{name}.png")
    cv2.imwrite(out, vis)
    return out
...
def main():
    ...
    p.add_argument("--vis", action="store_true", help="另存差異圖到 color_chk/")
    a = p.parse_args()
