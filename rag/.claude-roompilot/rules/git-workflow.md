...
> **現況：專案尚未 `git init`，本檔全部規範待 init 後生效。**
> 在 init 之前，下列 `git` / `gh` 指令**無法執行**；改動前請先手動備份會被覆寫的資料檔
> （`rag_dataset/`、`rag_export/`、`chroma_db/`、`vlm_annotation/*.jsonl`）。
> `.anthropic_key` 必須在第一個 commit 前就寫進 `.gitignore`，絕不可入庫。
...
（本專案無 CI，`ci` 型別保留給日後補建的自動化腳本；目前一般用不到。
另建議加 `data` 型別，用於 `rag_dataset/`／`rag_export/` 的資料版本異動。）
...
範例：
- `feat/query-parser-hyde`
- `fix/chroma-where-rag-indexable`
- `refactor/retriever-scoring-weights`
- `chore/update-taxonomy-v2`
...
- [ ] 所有測試通過（解析器 schema／檢索排序／資料加工／端到端 CLI 四類；
      pytest 套件尚未建置前，以 rules/testing.md 的手動驗證指令替代）
- [ ] commit 歷史已審計（WHY/WHAT/IMPACT body 完整）
- [ ] 已自我 review 完整 diff：`git diff <base>...HEAD`
- [ ] 無殘留 debug code（臨時 `print()`、TODO hack、註解掉的程式碼）
- [ ] 無金鑰／大型二進位入庫（`.anthropic_key`、`chroma_db/`、`rendering/output/` 應在 `.gitignore`）
...
- 資料版本另行標記：`furniture_enriched_vN.json` / collection `furniture_vN` / `taxonomy_vN.json`
  三者的版號需一致；換版時在 CHANGELOG 註明是否需重建索引
