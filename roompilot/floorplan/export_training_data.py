...
"""export_training_data.py — 21 張測資 → 分割模型訓練資料(交接峙宏 CubiCasa5k 分支)。
    ...
    uv run --extra vision python roompilot/floorplan/export_training_data.py <輸出目錄>
...
標注來源與品質(誠實聲明,訓練時要知道):
    ...
    窗  = testdata/pngans/ 的人工標準答案綠框(人工 GT,最可信)
...
def main() -> None:
    ...
    entries = []
    for png in sorted(glob.glob(os.path.join(ROOT, "testdata/png/floor*.png"))):
        base = os.path.splitext(os.path.basename(png))[0]
        ans = os.path.join(ROOT, f"testdata/pngans/{base}_ans.png")
    ...
    meta = {
        ...
        "label_sources": {
            ...
            "window": "human GT (testdata/pngans green boxes) — 最可信",
            "door": "machine (detect_doors score>=0.85, 方形近似) — 覆蓋率低",
        },
        "intended_use": "CubiCasa5k 預訓練模型的台灣圖風微調集+驗收集(21 張,不夠從零訓練)",
        "eval": "驗收請沿用 roompilot/floorplan/eval_windows.py(窗)與 eval_doors.py(門過濾)",
        "images": entries,
    }
