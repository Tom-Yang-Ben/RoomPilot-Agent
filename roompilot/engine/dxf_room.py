...
── 為什麼需要轉換:兩邊座標系、單位與牆體表示不同 ──────────────────────
  ...
  Room 引擎需要:
    ...
    - 單位一律公分(2026-07-08 全管線公分化)——本模組是「公尺→公分」的唯一邊界,
      上游 dxf_parser 的公尺輸出在這裡 ×100 進引擎。
...
# dxf_parser 輸出公尺,引擎一律公分——進門先放大(上游契約不動,見模組 docstring)
_M_TO_CM = 100.0
...
@dataclass
class DxfRoomBuild:
    """轉換結果。room 給引擎用(公分);offset 讓你把家具座標映射回平面座標:
    plan_x_cm = pos_x + offset[0]、plan_z_cm = pos_y + offset[1],再除以 100 得
    dxf_parser 的公尺平面座標(接 3D 全景時用得到)。"""
    room: Room
...
def build_room_from_dxf(
    ...
    """dxf_parser 的輸出 dict(公尺)→ DxfRoomBuild(公分)。單位換算在本函式完成。"""
