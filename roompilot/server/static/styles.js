import { fetchSiteData, formatList, formatTypeLabel, initBackgroundFx } from "./common.js?v=20260708a";
...
function countFurnitureForStyle(styleId) {
  return data.furniture.filter((item) => hasStyleTag(item, styleId)).length;
}
...
function renderStyleTagSummary(style) {
  const allItems = data.furniture || [];
  const taggedItems = allItems.filter((item) => positiveStyleCandidates(item).length > 0);
  const activeItems = allItems.filter((item) => hasStyleTag(item, style.style_id));
  const activeGroupCounts = countFurnitureGroups(activeItems);
  ...
  return `
    <div class="style-db-summary-grid compact">
      ...
      <div>
        <span>已具風格標籤</span>
        <strong>${taggedItems.length}/${allItems.length}</strong>
      </div>
    </div>
...
function renderTabs() {
  ...
  data.styles.forEach((style, index) => {
    const colors = style.palette_hex?.length ? style.palette_hex : ["#f4eadc", "#d7c2a8", "#a88462"];
