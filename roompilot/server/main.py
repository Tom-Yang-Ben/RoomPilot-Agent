...
from .floorplan_vlm import annotate_floorplan
...
async def recognize_floorplan(
    ...
    if suffix == ".dxf":
        ...
        try:
            ...
            floorplan = _flip_parsed_z(parse_dxf_bytes(raw, name, scale_m))
        except Exception as e:
    ...
    with tempfile.TemporaryDirectory(prefix="rp_recognize_") as tmp:
        ...
        try:
            # _flip_parsed_z:DXF y 朝北、three.js +z 朝南,不翻轉畫面會上下鏡像
            floorplan = _flip_parsed_z(parse_dxf_bytes(dxf_text.encode("utf-8"), name, scale_m))
        except Exception as e:
        ...
        # 失敗回 None,不影響幾何辨識結果。
        vlm_info = None
        vlm = annotate_floorplan(raw, mime=f"image/{'jpeg' if suffix in ('.jpg', '.jpeg') else 'png'}")
        if vlm:
            if handoff_data:
