...
- `Scene.jsx` — R3F render. `Walls` extrudes each `wall_poly` shape (with holes) to `wall_height`; `SegBoxes` lays a box along each window/door segment; `Floor` is the bbox slab. drei `<Bounds>` auto-fits the camera per plan, `<OrbitControls>`, `<Grid>`.
  - **X-ray walls** (`makeWallMaterial` + the `Walls` `useFrame`): zooming in fades the wall *surfaces* between the camera and the orbit target so you can see into rooms (the `近景穿牆` checkbox). Done per-fragment via `onBeforeCompile` (injects a `vWorldPos` varying + a view-ray distance test into the standard material) because the backend unions all walls into one mesh — front and back walls share it, so per-mesh opacity can't single out the near side. Thresholds scale with the plan diagonal. The walls are one big self-overlapping union mesh, so `depthWrite` is left ON (turning it off to see the far wall through the near one made the mesh's own faces blend in submission order and shimmer); a faded near wall instead blends its `uMinAlpha` over the opaque interior, which is stable. Near-vertical views fade the effect out (the view-ray test degenerates to a height test top-down).
